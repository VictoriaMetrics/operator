...
const (
	vlAgentQueueMetricName         = "vlagent_remotewrite_pending_data_bytes"
...
func CreateOrUpdate(ctx context.Context, cr *vmv1alpha1.VLDistributed, rclient client.Client) error {
	...
	// build VLDistributed desired zones state
	zs, err := getZones(ctx, rclient, cr)
	if err != nil {
		return fmt.Errorf("failed to build distributed zones VLDistributed=%s: %w", nsn.String(), err)
	}

	// Apply changes to VLClusters one by one if new spec needs to be applied
	...
	for i := range cr.Spec.Zones {
		...
		// Sleep for zoneUpdatePause time between VLClusters updates (unless its the last one)
		if i != lastZoneIdx && zs.hasChanges[i] {
			...
			case <-ctx.Done():
				return fmt.Errorf("update of VLDistributed=%s was canceled by controller", nsn.String())
			}
		}
	}
