...
var _ = Describe("VMAuth Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind VMAuth")
			err := k8sClient.Get(ctx, typeNamespacedName, vmauth)
			if err != nil && k8serrors.IsNotFound(err) {
				resource := &vmv1beta1.VMAuth{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance VMAuth")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &VMAuthReconciler{
				Client:       k8sClient,
...
func TestVMAuth_Reconcile_AgentSync_Managed(t *testing.T) {
	g := NewWithT(t)
	managed := &vmv1beta1.VMAuth{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "managed",
			Namespace: "default",
		},
	...
	// start with locked auth reconcile
	locked := true
	authSync.Lock()
	defer func() {
		if locked {
...
func TestVMAuth_Reconcile_AgentSync_Unmanaged(t *testing.T) {
	g := NewWithT(t)
	unmanaged := &vmv1beta1.VMAuth{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "unmanaged",
			Namespace: "default",
		},
