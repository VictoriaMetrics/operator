...
func TestCreateOrUpdateAlertManager(t *testing.T) {
	...
	f := func(opts opts) {
		...
		err := CreateOrUpdateAlertManager(ctx, opts.cr, fclient)
		if err != nil {
...
func Test_createDefaultAMConfig(t *testing.T) {
	...
	f := func(opts opts) {
		...
		fclient := k8stools.GetTestClientWithObjects(opts.predefinedObjects)
		ctx := context.TODO()
		if err := CreateOrUpdateConfig(ctx, fclient, opts.cr, nil); (err != nil) != opts.wantErr {
			t.Fatalf("createDefaultAMConfig() error = %v, wantErr %v", err, opts.wantErr)
		}
		if opts.wantErr {
			return
		}
		...
		if err != nil {
			if k8serrors.IsNotFound(err) && opts.secretMustBeMissing {
				return
			}
