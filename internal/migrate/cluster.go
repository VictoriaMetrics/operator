...
func Cluster[Cluster clusterCR, Agent bufferAgentCR](ctx context.Context, c client.Client, httpClient *http.Client, opts Options, target Cluster, engine ClusterEngine[Cluster, Agent], noDowntime bool) error {
	...
	insertHelmLabel := engine.componentLabel(vmv1beta1.ClusterComponentInsert)
	insertD, err := discoverComponent(ctx, c, opts.Namespace, opts.ReleaseName, insertHelmLabel)
	if err != nil {
	...
	case len(insertD.StatefulSets) == 1:
		...
		agentPathPrefix = p
	default:
		if probe, buildErr := engine.BuildAgent(agentName, target.GetNamespace(), targetWriteURL, opts.AgentBufferSize, ""); buildErr == nil {
			if getErr := c.Get(ctx, types.NamespacedName{Name: agentName, Namespace: target.GetNamespace()}, probe); getErr == nil {
				agentPathPrefix = probe.GetExtraArgs()["http.pathPrefix"]
			}
		}
	}
	agentWriteURL := engine.BuildAgentWriteURL(target, targetWriteURL)
	agent, err := engine.BuildAgent(agentName, target.GetNamespace(), agentWriteURL, opts.AgentBufferSize, agentPathPrefix)
	if err != nil {
	...
	for _, kind := range clusterComponentKinds {
		...
		if d.Deployment != nil {
			...
			if kind == vmv1beta1.ClusterComponentStorage {
				...
				wantReplicas := engine.StorageReplicaCount(target)
				switch {
				case engine.StorageUsesHPA(target):
					...
					if maxReplicas := engine.StorageMaxReplicas(target); len(d.PVCs) > int(maxReplicas) {
		...
		if requiresServiceCutover(kind) && len(cutoverCandidateServices(dc, target.SelectorLabels(kind), agentSelectorLabels, false)) == 0 {
	...
	for _, dc := range discovered {
		...
		for _, svc := range cutoverCandidateServices(dc, target.SelectorLabels(dc.kind), agentSelectorLabels, acceptWhenBaseLabelsNil) {
			if sibling, ok := selectorMatchesSiblingPods(svc.Spec.Selector, discovered, dc.kind); ok {
				return fmt.Errorf("component %q's Service %s/%s has a selector that also matches component %q's pods — refusing to redirect it during cutover and risk affecting unrelated traffic; narrow the Service's selector to this component's own pods, or repoint it manually",
					engine.componentLabel(dc.kind), svc.Namespace, svc.Name, engine.componentLabel(sibling))
			}
		}
	}
	...
	if insertDC, ok := findComponent(discovered, vmv1beta1.ClusterComponentInsert); ok {
		insertSvcPtrs = cutoverCandidateServices(insertDC, target.SelectorLabels(vmv1beta1.ClusterComponentInsert), agentSelectorLabels, false)
	}
	...
	for _, svc := range insertSvcPtrs {
		if err := verifyAgentPortCompatible(ctx, c, svc, agent); err != nil {
			return err
		}
	}
	...
	if dc, ok := findComponent(discovered, vmv1beta1.ClusterComponentStorage); ok {
		...
		if !dc.resuming {
			storageSourceCount = len(dc.d.PVCs)
		}
	}
	var allDependentConfigMaps []corev1.ConfigMap
	var allDependentSecrets []corev1.Secret
	for _, dc := range discovered {
		...
		case dc.resuming:
			if len(d.PVCs) == 0 {
				...
				if pvcTemplateName := engine.PVCTemplateName(target, dc.kind); pvcTemplateName != "" {
					namePrefix := pvcTemplateName + "-" + target.PrefixedName(dc.kind) + "-"
					if err := ensureClusterReclaimPolicyRestored(ctx, c, target.GetNamespace(), target.SelectorLabels(dc.kind), namePrefix); err != nil {
						return fmt.Errorf("component %q: %w", helmLabel, err)
					}
				}
				continue
			...
			if err := finishInterruptedRebind(ctx, c, d.PVCs, engine.PVCTemplateName(target, dc.kind), target.PrefixedName(dc.kind), target.GetNamespace(), target.SelectorLabels(dc.kind)); err != nil {
		...
		case d.Deployment != nil:
			...
			allDependentConfigMaps = append(allDependentConfigMaps, dependentConfigMaps...)
			allDependentSecrets = append(allDependentSecrets, dependentSecrets...)
		...
		default:
			sts := dc.sts
			if len(d.PVCs) > 0 {
				if err := checkTargetPVCsRebindable(ctx, c, d.PVCs, engine.PVCTemplateName(target, dc.kind), target.PrefixedName(dc.kind), target.GetNamespace()); err != nil {
					return fmt.Errorf("component %q: %w", helmLabel, err)
				}
			}
			...
			if err := rebindClusterPVCs(ctx, c, d.PVCs, engine.PVCTemplateName(target, dc.kind), target.PrefixedName(dc.kind), target.GetNamespace(), target.SelectorLabels(dc.kind)); err != nil {
				return fmt.Errorf("component %q (writes are safely buffered on disk): %w", helmLabel, err)
			}
		}
	}
	...
	if !noDowntime && storageWorkloadName != "" {
		wantReplicas := engine.StorageReplicaCount(target)
		if storageSourceCount > int(wantReplicas) {
			wantReplicas = int32(storageSourceCount)
		}
		if err := verifyRebindComplete(ctx, c, wantReplicas, storagePVCTemplateName, storageWorkloadName, target.GetNamespace()); err != nil {
	...
	if len(allDependentConfigMaps) > 0 || len(allDependentSecrets) > 0 {
		if err := deleteDependentConfigs(ctx, c, target, allDependentConfigMaps, allDependentSecrets); err != nil {
			return err
		}
	}
	...
	for _, dc := range discovered {
		...
		svcPtrs := cutoverCandidateServices(dc, target.SelectorLabels(dc.kind), agentSelectorLabels, true)
