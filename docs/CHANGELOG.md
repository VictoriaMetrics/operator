...
* FEATURE: [vmoperator](https://docs.victoriametrics.com/operator/): add `spec.serviceSpecs` (a `map[string]AdditionalServiceSpec`) to every CRD that previously had a singular `spec.serviceSpec`, replacing the single override slot with named, per-port Service overrides. The reserved key `"default"` merges into the CR's own primary Service, same as `serviceSpec.useAsDefault: true` did; any other key must name a port the component actually exposes (e.g. `http`, or a cluster-internal port like `clusternative`/`mesh`/`vminsert`/`vmselect`) and creates a separate Service carrying only that port — cluster-internal ports are forced headless automatically. `spec.serviceSpec` keeps working unchanged when `spec.serviceSpecs` is unset; setting `useAsDefault` inside a `serviceSpecs` entry is now rejected, since the `"default"` key replaces it. `VLSelect`/`VLInsert`/`VTSelect`/`VTInsert` gain a `clusternative`-named Service port (aliasing their existing port) so they can be split out the same way `VMSelect`/`VMInsert` already could. `spec.serviceSpec` is now deprecated in favor of `spec.serviceSpecs`.
