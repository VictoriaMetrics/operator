...
func TestCreateOrUpdate(t *testing.T) {
	f := func(cr *vmv1beta1.VMCluster, predefinedObjects []runtime.Object) {
		t.Helper()
		...
		if cr.Spec.VMInsert != nil {
			...
			eventuallyUpdateStatusToOk(func() error {
				if err := fclient.Get(ctx, types.NamespacedName{Name: cr.GetVMInsertName(), Namespace: cr.Namespace}, &vminsert); err != nil {
					return err
				}
		...
		if cr.Spec.VMSelect != nil {
			...
			eventuallyUpdateStatusToOk(func() error {
				if err := fclient.Get(ctx, types.NamespacedName{Name: cr.GetVMSelectName(), Namespace: cr.Namespace}, &vmselect); err != nil {
					return err
				}
				vmselect.Status.ReadyReplicas = *cr.Spec.VMSelect.ReplicaCount
				vmselect.Status.UpdatedReplicas = *cr.Spec.VMSelect.ReplicaCount
		...
		if cr.Spec.VMStorage != nil {
			...
			eventuallyUpdateStatusToOk(func() error {
				if err := fclient.Get(ctx, types.NamespacedName{Name: cr.GetVMStorageName(), Namespace: cr.Namespace}, &vmstorage); err != nil {
					return err
				}
				vmstorage.Status.ReadyReplicas = *cr.Spec.VMStorage.ReplicaCount
				vmstorage.Status.UpdatedReplicas = *cr.Spec.VMStorage.ReplicaCount
