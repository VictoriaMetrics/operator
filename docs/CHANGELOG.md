...
* BUGFIX: [vmoperator](https://docs.victoriametrics.com/operator/): report `status.updateStatus: operational` for config-only objects - `VMRule`, `VMUser`, `VMAlertmanagerConfig`, `VMAnomalyConfig`, `VMServiceScrape`, `VMPodScrape`, `VMNodeScrape`, `VMProbe`, `VMStaticScrape` and `VMScrapeConfig` - regardless of what the applications selecting them decided. It is set to `failed` only when the operator cannot parse the object's own spec, and the `ignored` value is no longer used. The state reported by each application is available at the `<name>.<namespace>.<resource>.victoriametrics.com/Applied` conditions at `status.conditions`. See [#2649](https://github.com/VictoriaMetrics/operator/issues/2649).
