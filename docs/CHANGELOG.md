...
* BUGFIX: [vmalert](https://docs.victoriametrics.com/operator/resources/vmalert/): stop reshuffling unrelated `VMRule` groups across rule ConfigMap buckets whenever a group was added or removed elsewhere in a large rule set. vmalert derives a group's identity from the file it's stored in, so a reshuffled group was treated as brand new and lost its evaluation state and metrics on every such change. Rule groups now keep their previous bucket whenever it still fits them. See [#2610](https://github.com/VictoriaMetrics/operator/issues/2610).
