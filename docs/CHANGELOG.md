...
* BUGFIX: [vmanomaly](https://docs.victoriametrics.com/operator/resources/vmanomaly/): preserve an explicit `0.0` for the `anomaly_score_outside_data_range` parameter (both `settings` and model level) and for online models' [`decay`](https://docs.victoriametrics.com/anomaly-detection/components/models/#decay). These valid values were dropped on the underlying float fields, silently reverting to vmanomaly defaults.
