...
func TestClusterEngine_BuildAgentWriteURL_UsesTargetOwnPathPrefix(t *testing.T) {
	target := &vmv1.VLCluster{
		ObjectMeta: metav1.ObjectMeta{Name: "myrelease", Namespace: "default"},
		Spec: vmv1.VLClusterSpec{
			VLInsert: &vmv1.VLInsert{CommonAppsParams: vmv1beta1.CommonAppsParams{ExtraArgs: map[string]string{"http.pathPrefix": "/target-prefix"}}},
		},
	}

	engine := clusterEngine()
	got := engine.BuildAgentWriteURL(target, "http://vlinsert-myrelease.default.svc:9481/insert/native")
	assert.Equal(t, "http://vlinsert-myrelease.default.svc:9481/target-prefix/internal/insert", got)
}
