...
func TestCreateOrUpdate(t *testing.T) {
	f := func(cr *vmv1.VMAnomaly, validate func(set *appsv1.StatefulSet) error) {
		t.Helper()
...
func Test_createDefaultConfig(t *testing.T) {
	f := func(cr *vmv1.VMAnomaly, wantErr bool, predefinedObjects []runtime.Object) {
		t.Helper()
		...
		if _, err := createOrUpdateConfig(ctx, fclient, cr, nil, ac); (err != nil) != wantErr {
			t.Fatalf("createOrUpdateConfig() error = %v, wantErr %v", err, wantErr)
		}
		if wantErr {
			return
		}
