...
// TestNoDowntimeSingleNode exercises the full flow against a fake client + a fake HTTP
// server standing in for both the old VictoriaMetrics storage's admin API and the buffer
// VMAgent's /metrics endpoint. Everything except the CSI driver itself (simulated here by
// manually flipping VolumeSnapshot.status.readyToUse, since no real snapshot controller runs
// against a fake client) is exercised end-to-end.
func TestNoDowntimeSingleNode(t *testing.T) {
	...
	migrate.EndpointsReadyPollInterval = 5 * time.Millisecond
	migrate.EndpointsReadyTimeout = time.Second
	migrate.WriteQuiesceGracePeriod = time.Millisecond
	...
	// production code addresses these via in-cluster DNS names, which don't resolve in a
	// unit test process; redirect every dial to the fake server's real address instead.
	...
	go simulateAgentEndpoints(ctx, c, ns, target.PrefixedName()+"-migration-buffer", adminServer.URL)
	go simulateVMAgentOperational(ctx, c, types.NamespacedName{Name: target.PrefixedName() + "-migration-buffer", Namespace: ns})
	simulateServiceEndpoints(ctx, c, ns, target.PrefixedName(), adminServer.URL)
	...
	assert.Contains(t, initialWriteURL, "-migration-source")
...
func TestNoDowntimeSingleNode_StaleBufferAgentIsAnError(t *testing.T) {
	...
	// nothing beyond agent creation must have happened: old Deployment/PVC/Service untouched.
	var checkDep appsv1.Deployment
	assert.NoError(t, c.Get(ctx, types.NamespacedName{Name: release, Namespace: ns}, &checkDep))
...
// freshly-created VMSingle Operational, which the fake client won't do on its own.
...
// simulateVMAgentOperational mimics the operator's own reconcile loop marking a freshly-created
// buffer VMAgent Operational, which the fake client won't do on its own.
func simulateVMAgentOperational(ctx context.Context, c client.Client, nsn types.NamespacedName) {
	for {
		var cr vmv1beta1.VMAgent
		if err := c.Get(ctx, nsn, &cr); err == nil {
			cr.Status.UpdateStatus = vmv1beta1.UpdateStatusOperational
			cr.Status.ObservedGeneration = cr.Generation
			_ = c.Status().Update(ctx, &cr)
...
// something to poll.
func simulateAgentEndpoints(ctx context.Context, c client.Client, namespace, agentName, adminServerURL string) {
...
func simulateServiceEndpoints(ctx context.Context, c client.Client, namespace, serviceName, adminServerURL string) {
	u, err := parseHostPort(adminServerURL)
	if err != nil {
		return
	}
