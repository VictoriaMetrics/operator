...
func VMProbe(ctx context.Context, rclient client.Client, newObj, prevObj *vmv1beta1.VMProbe, owner *metav1.OwnerReference, isConversion bool) error {
	...
	return retryOnConflict(func() error {
		var existingObj vmv1beta1.VMProbe
		if err := rclient.Get(ctx, nsn, &existingObj); err != nil {
			if k8serrors.IsNotFound(err) {
				logger.WithContext(ctx).Info(fmt.Sprintf("creating VMProbe=%s", nsn.String()))
				return rclient.Create(ctx, newObj)
			}
			return err
		}
		if isConversion && existingObj.Annotations[vmv1beta1.IgnoreConversionLabel] == vmv1beta1.IgnoreConversion {
			logger.WithContext(ctx).Info(fmt.Sprintf("syncing for VMProbe=%s was disabled by annotation", nsn.String()))
			return nil
		}
		...
		logger.WithContext(ctx).Info(fmt.Sprintf("updating VMProbe %s", strings.Join(logMessageMetadata, ", ")))
