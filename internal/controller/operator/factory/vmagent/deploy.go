...
func newDeploy(cr *vmv1beta1.VMAgent, ssCache *scrapesSecretsCache) (runtime.Object, error) {
	...
	if cr.Spec.DaemonSetMode {
		...
		build.DaemonSetAddCommonParams(app, useStrictSecurity, &cr.Spec.CommonApplicationDeploymentParams)
	...
	if cr.Spec.StatefulMode {
		...
		app.Spec.Template.Spec.Volumes = build.AddServiceAccountTokenVolume(app.Spec.Template.Spec.Volumes, &cr.Spec.CommonApplicationDeploymentParams)
		cr.Spec.StatefulStorage.IntoStatefulSetVolume(cr.GetVolumeName(), &app.Spec)
