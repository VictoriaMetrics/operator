...
func createOrUpdateVLSelect(ctx context.Context, rclient client.Client, cr, prevCR *vmv1.VLCluster) error {
	...
	if !ptr.Deref(cr.Spec.VLSelect.DisableSelfServiceScrape, false) {
		...
		if err != nil {
			return fmt.Errorf("cannot create VMServiceScrape for vSelect: %w", err)
		}
	}
...
func createOrUpdateVLSelectService(ctx context.Context, rclient client.Client, cr, prevCR *vmv1.VLCluster) (*corev1.Service, error) {
	...
	if err := cr.Spec.VLSelect.ServiceSpec.IsSomeAndThen(func(s *vmv1beta1.AdditionalServiceSpec) error {
		...
		if err := reconcile.Service(ctx, rclient, additionalService, prevAdditionalService); err != nil {
			return fmt.Errorf("cannot reconcile service for vlselect: %w", err)
		}
		return nil
	}); err != nil {
		return nil, err
	...
	if err := reconcile.Service(ctx, rclient, svc, prevService); err != nil {
		return nil, fmt.Errorf("cannot reconcile vlselect service: %w", err)
	}
	if cr.Spec.RequestsLoadBalancer.Enabled && !cr.Spec.RequestsLoadBalancer.DisableSelectBalancing {
		var prevPort string
		if prevCR != nil && prevCR.Spec.VLSelect != nil {
		...
		if err := createOrUpdateLBProxyService(ctx, rclient, cr, prevCR, cr.GetVLSelectName(), cr.Spec.VLSelect.Port, prevPort, "vlselect", cr.VMAuthLBSelectorLabels()); err != nil {
			return nil, fmt.Errorf("cannot create lb svc for vlselect: %w", err)
		}
	}
	return svc, nil
}
...
func createOrUpdateVLSelectSTS(ctx context.Context, rclient client.Client, cr, prevCR *vmv1.VLCluster) error {
	...
	if prevCR != nil && prevCR.Spec.VLSelect != nil {
		var err error
		prevSts, err = buildVLSelectSTS(prevCR)
		if err != nil {
			return fmt.Errorf("cannot build prev storage spec: %w", err)
		}
	}
	newSts, err := buildVLSelectSTS(cr)
	if err != nil {
		return err
	}
	...
	stsOpts := reconcile.STSOptions{
		...
		HPA:            cr.Spec.VLSelect.HPA,
		UpdateReplicaCount: func(count *int32) {
			if cr.Spec.VLSelect.HPA != nil && count != nil {
				cr.Spec.VLSelect.ReplicaCount = count
			}
		},
	}
...
func buildVLSelectSTS(cr *vmv1.VLCluster) (*appsv1.StatefulSet, error) {
	...
	stsSpec := &appsv1.StatefulSet{
		...
		Spec: appsv1.StatefulSetSpec{
			...
			UpdateStrategy: appsv1.StatefulSetUpdateStrategy{
				Type: cr.Spec.VLSelect.RollingUpdateStrategy,
			},
			Template:    *podSpec,
			ServiceName: cr.GetVLSelectName(),
		},
	}
	build.StatefulSetAddCommonParams(stsSpec, ptr.Deref(cr.Spec.VLSelect.UseStrictSecurity, false), &cr.Spec.VLSelect.CommonApplicationDeploymentParams)
	stsSpec.Spec.VolumeClaimTemplates = append(stsSpec.Spec.VolumeClaimTemplates, cr.Spec.VLSelect.ClaimTemplates...)
	return stsSpec, nil
}
