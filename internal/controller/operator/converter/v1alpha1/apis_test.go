...
func TestConvertAlertmanagerConfig(t *testing.T) {
	type opts struct {
		cfg      *promv1alpha1.AlertmanagerConfig
		validate func(convertedAMCfg *vmv1beta1.VMAlertmanagerConfig) error
	}
	f := func(opts opts) {
		t.Helper()
		converted, err := ConvertAlertmanagerConfig(opts.cfg, &config.BaseOperatorConf{})
		if err != nil {
...
func TestConvertScrapeConfig(t *testing.T) {
	type opts struct {
		cfg      *promv1alpha1.ScrapeConfig
		ownerRef bool
		want     vmv1beta1.VMScrapeConfig
	}
	f := func(opts opts) {
		t.Helper()
		got := ConvertScrapeConfig(opts.cfg, &config.BaseOperatorConf{EnabledPrometheusConverterOwnerReferences: opts.ownerRef})
