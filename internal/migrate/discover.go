...
func discover(ctx context.Context, c client.Client, namespace, releaseName string, matchLabels map[string]string) (*Discovered, error) {
	...
	opts := &client.ListOptions{Namespace: namespace, LabelSelector: sel}
	list := func(list client.ObjectList, kind string) error {
		if err := c.List(ctx, list, opts); err != nil {
			return fmt.Errorf("cannot list %s for release %q: %w", kind, releaseName, err)
		}
		return nil
	}

	var deployments appsv1.DeploymentList
	if err := list(&deployments, "deployments"); err != nil {
		return nil, err
	}
	var statefulSets appsv1.StatefulSetList
	if err := list(&statefulSets, "statefulsets"); err != nil {
		return nil, err
	}
	var services corev1.ServiceList
	if err := list(&services, "services"); err != nil {
		return nil, err
	}
	var pvcs corev1.PersistentVolumeClaimList
	if err := list(&pvcs, "persistentvolumeclaims"); err != nil {
		return nil, err
	}
	...
	if err := list(&configMaps, "configmaps"); err != nil {
		return nil, err
	}
	var secrets corev1.SecretList
	if err := list(&secrets, "secrets"); err != nil {
		return nil, err
	}
