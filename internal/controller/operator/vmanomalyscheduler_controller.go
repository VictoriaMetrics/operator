...
func (r *VMAnomalySchedulerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	if err = r.Get(ctx, req.NamespacedName, instance); err != nil {
		err = &getError{err, "vmanomalyscheduler", req}
		return
	}

	RegisterObjectStat(instance, "vmanomalyscheduler")
	if instance.Spec.ParsingError != "" {
		err = &parsingError{instance.Spec.ParsingError, "vmanomalyscheduler"}
		return
	}
	...
	if err = k8stools.ListObjectsByNamespace(ctx, r.Client, r.BaseConf.WatchNamespaces, func(dst *vmv1.VMAnomalyList) {
		...
		err = fmt.Errorf("cannot list vmanomalies for vmanomalyscheduler: %w", err)
		return
	}
...
func (r *VMAnomalySchedulerReconciler) IsDisabled(_ *config.BaseOperatorConf, disabledControllers sets.Set[string]) bool {
