...
func TestCreateAliasService(t *testing.T) {
	...
	source := &corev1.Service{Spec: corev1.ServiceSpec{Type: corev1.ServiceTypeClusterIP, Ports: ports}}

	c := k8stools.GetTestClientWithObjects(nil)
	ctx := context.Background()

	svc, err := createAliasService(ctx, c, "myrelease-migration-source", ns, source, selector)
	...
	svc2, err := createAliasService(ctx, c, "myrelease-migration-source", ns, source, selector)
	require.NoError(t, err)
	assert.Equal(t, svc.UID, svc2.UID)
	assert.Equal(t, svc.ResourceVersion, svc2.ResourceVersion)
	assert.Equal(t, selector, svc2.Spec.Selector)
	assert.Equal(t, ports, svc2.Spec.Ports)
}
...
func TestCreateAliasService_ClearsNodePort(t *testing.T) {
	...
	selector := map[string]string{"app": "real-old-storage"}
	source := &corev1.Service{Spec: corev1.ServiceSpec{Ports: ports}}
...
func TestCreateAliasService_MismatchedExistingServiceIsAnError(t *testing.T) {
	...
	_, err := createAliasService(ctx, c, "myrelease-migration-source", ns, source, selector)
	assert.ErrorContains(t, err, "selector/ports don't match")
}
...
func TestCreateAliasService_PreservesSourceType(t *testing.T) {
	ns := "default"
	ports := []corev1.ServicePort{{Name: "http", Port: 8428, NodePort: 30080}}
	selector := map[string]string{"app": "real-old-storage"}
	...
	svc, err := createAliasService(ctx, c, "myrelease-migration-source", ns, source, selector)
	require.NoError(t, err, "external clients relying on the source Service's NodePort exposure must get the same exposure on the alias")
...
func TestCreateAliasService_RejectsExternalIPs(t *testing.T) {
	ns := "default"
	ports := []corev1.ServicePort{{Name: "http", Port: 8428}}
	selector := map[string]string{"app": "real-old-storage"}
	source := &corev1.Service{
	...
	_, err := createAliasService(ctx, c, "myrelease-migration-source", ns, source, selector)
	assert.ErrorContains(t, err, "externalIPs", "duplicating externalIPs onto a second Service would create undefined routing")
}
...
func TestCreateAliasService_SurvivesClientFacingServiceSelectorChange(t *testing.T) {
	...
	aliasSvc, err := createAliasService(ctx, c, "myrelease-migration-source", ns, clientFacingSvc, clientFacingSvc.Spec.Selector)
	require.NoError(t, err)
