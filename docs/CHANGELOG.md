...
**Update note 1**: [vmoperator](https://docs.victoriametrics.com/operator/): removed `spec.useLegacyNaming`, introduced in v0.73.0, from all CRDs. Toggling it was already unsafe on its own: since it changes the naming convention for StatefulSets/Deployments/PVCs, flipping it on an existing CR causes the operator to rename/recreate those resources, and for PVCs specifically this meant binding to a *different*, empty PersistentVolumeClaim rather than migrating any existing data — a footgun the operator never covered and that would have taken significant work to handle safely (PV/PVC adoption, storage rebinding). The new `helm-converter migrate` command (see below) replaces its intended use case: it migrates by explicitly discovering and rebinding/repointing existing resources by identity, rather than relying on naming convention. Any existing CR with `useLegacyNaming: true` will have the field silently dropped by the updated CRD schema and revert to the operator's standard naming on the next reconcile — pin to the previous operator version until you've completed migration, or re-migrate using the new tool.

* FEATURE: [helm-converter](https://docs.victoriametrics.com/operator/helm-converter/): add a `migrate` command that automates cutover from a standalone Helm-chart-deployed installation to an operator-managed CR, with two strategies:
  * `WithDowntime` (`victoria-metrics-single`, `victoria-logs-single`, `victoria-metrics-cluster`, `victoria-logs-cluster`): deletes the old workload(s), rebinds existing PersistentVolume(s) under the operator's own PVC naming (no data copy), creates the target CR, waits for it to become ready, then repoints the release's existing Service(s) at the new pods. For cluster charts, this runs per-component (`vmstorage`/`vmselect`/`vminsert`, `vlstorage`/`vlselect`/`vlinsert`), rebinding each StatefulSet component's PVCs one per ordinal.
  * `NoDowntime` (`victoria-metrics-single`, `victoria-logs-single`, `victoria-metrics-cluster`, `victoria-logs-cluster`): never touches the old workload(s) or their storage. Deploys a buffering VMAgent (VLAgent for VictoriaLogs) in front of the write path, redirects incoming writes through it, takes a CSI VolumeSnapshot of the old storage to seed fresh storage for the target CR, backfills the buffered writes once it's ready, then cuts traffic over — the old release is left fully intact for you to decommission separately (e.g. `helm uninstall`) whenever you're ready. For cluster charts, only the insert component (`vminsert`/`vlinsert`) is buffered; the select component (`vmselect`/`vlselect`) cuts over directly (nothing to buffer on the read path), and the storage component (`vmstorage`/`vlstorage`, no client-facing Service) has its per-ordinal PVCs force-merged and snapshotted directly.
...
* BUGFIX: [helm-converter](https://docs.victoriametrics.com/operator/helm-converter/): fix `securityContext.runAsNonRoot`, `runAsUser`, and `seccompProfile` being silently dropped during conversion; these are now promoted to `spec.securityContext.podSecurityContext`, matching how the operator applies them to containers. See [#2391](https://github.com/VictoriaMetrics/operator/issues/2391).
