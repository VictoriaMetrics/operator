...
* BUGFIX: [helm-converter](https://docs.victoriametrics.com/operator/helm-converter/): fix an invalid `spec.notifier: {}` being generated for the `victoria-metrics-alert` chart when no notifier URL is set, which the operator webhook rejects. Such a notifier is now omitted, and the conversion fails with an explicit error when the chart's bundled `alertmanager` would have been used instead. The chart's top-level `license` block is now converted into `spec.license`. See [#2601](https://github.com/VictoriaMetrics/operator/issues/2601).
