...
func resumeSingleNodePVCFixture(release, ns string, reboundPVCAnnotations map[string]string, claimRef *corev1.ObjectReference) (*corev1.PersistentVolume, *corev1.PersistentVolumeClaim, *corev1.Service, *vmv1beta1.VMSingle) {
	pv := &corev1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{Name: "pv-data"},
		Spec: corev1.PersistentVolumeSpec{
			Capacity:    corev1.ResourceList{corev1.ResourceStorage: resource.MustParse("10Gi")},
	...
	reboundPVC := &corev1.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{Name: "vmsingle-" + release, Namespace: ns, Annotations: reboundPVCAnnotations},
		Spec: corev1.PersistentVolumeClaimSpec{
			AccessModes: []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
			VolumeName:  "pv-data",
		},
	...
	oldSvc := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name: release, Namespace: ns, Labels: helmLabels(release),
			Annotations: map[string]string{serviceCutoverAnnotation: "true"},
		},
		Spec: corev1.ServiceSpec{Selector: map[string]string{"app": release}},
	}
	...
	return pv, reboundPVC, oldSvc, target
}

func resumeSingleNode(t *testing.T, pv *corev1.PersistentVolume, reboundPVC *corev1.PersistentVolumeClaim, oldSvc *corev1.Service, target *vmv1beta1.VMSingle, release, ns string) error {
	t.Helper()
	scheme := runtime.NewScheme()
	utilruntime.Must(clientgoscheme.AddToScheme(scheme))
	utilruntime.Must(vmv1beta1.AddToScheme(scheme))
	...
	defer cancel()
	return SingleNode(ctx, c, handlerClient(testAdminHandler()), Options{
		Chart:       ChartVMSingle,
		Strategy:    StrategyWithDowntime,
		Namespace:   ns,
		ReleaseName: release,
...
func TestWithDowntimeSingleNode_ResumeRejectsPVCWithoutReboundAnnotation(t *testing.T) {
	release, ns := "myrelease", "default"
	claimRef := &corev1.ObjectReference{Kind: "PersistentVolumeClaim", Namespace: ns, Name: "vmsingle-" + release}
	pv, reboundPVC, oldSvc, target := resumeSingleNodePVCFixture(release, ns, nil, claimRef)

	err := resumeSingleNode(t, pv, reboundPVC, oldSvc, target, release, ns)
	assert.ErrorContains(t, err, "wasn't created by this tool's rebindPVC")
}

func TestWithDowntimeSingleNode_ResumeRejectsPVCWithBrokenClaimRef(t *testing.T) {
	release, ns := "myrelease", "default"
	annotations := map[string]string{reboundFromAnnotation: ns + "/some-other-release"}
	claimRef := &corev1.ObjectReference{Kind: "PersistentVolumeClaim", Namespace: ns, Name: "someone-elses-pvc"}
	pv, reboundPVC, oldSvc, target := resumeSingleNodePVCFixture(release, ns, annotations, claimRef)

	err := resumeSingleNode(t, pv, reboundPVC, oldSvc, target, release, ns)
	assert.ErrorContains(t, err, "claimRef")
}
