...
func makeSpecForVLogs(_ context.Context, r *vmv1beta1.VLogs, c *config.BaseOperatorConf) (*corev1.PodTemplateSpec, error) {
	...
	args = build.AppendArgsForInsertPorts(args, r.Spec.InsertPorts)
	...
	ports = build.AppendInsertPorts(ports, r.Spec.InsertPorts)
...
// CreateOrUpdateVLogsService creates service for vmlogs
func CreateOrUpdateVLogsService(ctx context.Context, r *vmv1beta1.VLogs, rclient client.Client, c *config.BaseOperatorConf) (*corev1.Service, error) {
	newService := build.Service(r, r.Spec.Port, func(svc *corev1.Service) {
		build.AppendInsertPortsToService(r.Spec.InsertPorts, svc)
	})
	...
	if err := r.Spec.ServiceSpec.IsSomeAndThen(func(s *vmv1beta1.AdditionalServiceSpec) error {
		...
		if additionalService.Name == newService.Name {
			logger.WithContext(ctx).Error(fmt.Errorf("vmlogs additional service name: %q cannot be the same as crd.prefixedname: %q", additionalService.Name, newService.Name), "cannot create additional service")
