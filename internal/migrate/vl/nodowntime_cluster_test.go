...
func TestNoDowntimeCluster(t *testing.T) {
	...
	go simulateVLAgentOperational(ctx, c, types.NamespacedName{Name: agentName, Namespace: ns})
	...
	simulateServiceEndpoints(ctx, c, ns, target.PrefixedName(vmv1beta1.ClusterComponentInsert), adminServer.URL)
	simulateServiceEndpoints(ctx, c, ns, target.PrefixedName(vmv1beta1.ClusterComponentSelect), adminServer.URL)
