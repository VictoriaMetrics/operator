...
func TestVTSingle_FinalLabels(t *testing.T) {
	type opts struct {
		cr           *VTSingle
	...
	// no common labels
	f(opts{
		cr: &VTSingle{ObjectMeta: metav1.ObjectMeta{Name: "test"}},
		want: map[string]string{
	...
	f(opts{
		cr: &VTSingle{
			...
			Spec:       VTSingleSpec{ManagedMetadata: &vmv1beta1.ManagedObjectsMetadata{Labels: map[string]string{"team": "backend"}}},
		},
...
func TestVTSingle_FinalAnnotations(t *testing.T) {
	type opts struct {
		cr                *VTSingle
	...
	// no annotations
	f(opts{cr: &VTSingle{ObjectMeta: metav1.ObjectMeta{Name: "test"}}, want: nil})
	...
	f(opts{
		cr: &VTSingle{
			ObjectMeta: metav1.ObjectMeta{Name: "test"},
			Spec:       VTSingleSpec{ManagedMetadata: &vmv1beta1.ManagedObjectsMetadata{Annotations: map[string]string{"note": "from-spec"}}},
		},
