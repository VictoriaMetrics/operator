...
func TestVLSingle_FinalLabels(t *testing.T) {
	type opts struct {
		cr           *VLSingle
	...
	// no common labels
	f(opts{
		cr: &VLSingle{ObjectMeta: metav1.ObjectMeta{Name: "test"}},
		want: map[string]string{
	...
	f(opts{
		cr: &VLSingle{
			...
			Spec:       VLSingleSpec{ManagedMetadata: &vmv1beta1.ManagedObjectsMetadata{Labels: map[string]string{"team": "backend"}}},
		},
...
func TestVLSingle_FinalAnnotations(t *testing.T) {
	type opts struct {
		cr                *VLSingle
	...
	// no annotations
	f(opts{cr: &VLSingle{ObjectMeta: metav1.ObjectMeta{Name: "test"}}, want: nil})
	...
	f(opts{
		cr: &VLSingle{
			ObjectMeta: metav1.ObjectMeta{Name: "test"},
			Spec:       VLSingleSpec{ManagedMetadata: &vmv1beta1.ManagedObjectsMetadata{Annotations: map[string]string{"note": "from-spec"}}},
		},
