...
func generateVMAuthConfig(cr *vmv1beta1.VMAuth, sus *skipableVMUsers, crdCache map[string]string, ac *build.AssetsCache) ([]byte, error) {
	...
	if cr.Spec.JWT != nil {
		...
		if len(jwt.PublicKeyFiles) > 0 {
			publicKeyFiles = append(publicKeyFiles, jwt.PublicKeyFiles...)
		}
		for _, ref := range jwt.PublicKeySecrets {
		...
		if len(publicKeyFiles) > 0 {
			jwtCfg = append(jwtCfg, yaml.MapItem{Key: "public_key_files", Value: publicKeyFiles})
		}
		if len(jwt.JWKSEndpoints) > 0 {
			jwtCfg = append(jwtCfg, yaml.MapItem{Key: "jwks_endpoints", Value: jwt.JWKSEndpoints})
		}
		if len(jwt.OIDCEndpoints) > 0 {
			jwtCfg = append(jwtCfg, yaml.MapItem{Key: "oidc_endpoints", Value: jwt.OIDCEndpoints})
		}
		if jwt.EnforcePrefix {
			jwtCfg = append(jwtCfg, yaml.MapItem{Key: "enforce_prefix", Value: jwt.EnforcePrefix})
		}
