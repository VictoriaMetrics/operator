...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/operator/resources/vmagent/), [vmsingle](https://docs.victoriametrics.com/operator/resources/vmsingle/), [vmalert](https://docs.victoriametrics.com/operator/resources/vmalert/): fix `config-reloader` sidecar crashing with `--target-dir count must match watched directory count` when `spec.configMaps` was combined with another feature that also makes the reloader watch a directory (extra scrape config secrets for `VMAgent`/`VMSingle`, or rule files for `VMAlert`). See [#2583](https://github.com/VictoriaMetrics/operator/issues/2583).
