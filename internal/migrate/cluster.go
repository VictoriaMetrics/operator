...
func cutoverCandidateServices(dc discoveredComponent, targetSelectorLabels, agentSelectorLabels map[string]string, acceptWhenBaseLabelsNil bool) []*corev1.Service {
	...
	for i := range dc.d.Services {
		...
		matchesBase := dc.baseLabels != nil && !selectorDiffersFromBase(selector, dc.baseLabels)
		...
		acceptsNilBase := dc.baseLabels == nil && acceptWhenBaseLabelsNil
...
func Cluster[Cluster clusterCR, Agent bufferAgentCR](ctx context.Context, c client.Client, httpClient *http.Client, opts Options, target Cluster, engine ClusterEngine[Cluster, Agent], noDowntime bool) error {
	...
	for _, kind := range clusterComponentKinds {
		...
		if d.Deployment == nil && len(d.StatefulSets) == 0 {
			...
			if requiresServiceCutover(kind) {
				...
				case 1:
					if kind == vmv1beta1.ClusterComponentInsert && d.Services[0].Annotations[serviceCutoverAnnotation] != "true" {
						return fmt.Errorf("component %q: old workload is already gone (resuming a previous attempt), but Service %s/%s wasn't previously redirected to the buffer agent by this tool (missing %q annotation) — refusing to repoint what may be an unrelated, coincidentally-labeled Service; investigate why the old workload is missing, or repoint it manually", helmLabel, d.Services[0].Namespace, d.Services[0].Name, serviceCutoverAnnotation)
					}
				default:
		...
		if requiresServiceCutover(kind) && !hasCutoverCandidate(d.Services, dc.baseLabels) &&
			!hasCutoverCandidate(d.Services, target.SelectorLabels(kind)) &&
	...
	for _, dc := range discovered {
		...
		for i := range dc.d.Services {
			selector := dc.d.Services[i].Spec.Selector
			if dc.baseLabels != nil && selectorDiffersFromBase(selector, dc.baseLabels) && selectorDiffersFromBase(selector, target.SelectorLabels(dc.kind)) {
...
func verifyRebindComplete(ctx context.Context, c client.Client, wantReplicas int32, targetPVCTemplateName, targetWorkloadName, targetNamespace string) error {
	for i := 0; i < int(wantReplicas); i++ {
		...
		if !strings.HasPrefix(pvc.Annotations[reboundFromAnnotation], targetNamespace+"/") {
			return fmt.Errorf("target PVC %s/%s (ordinal %d) exists but wasn't created by this migration's rebind from a source PVC in this namespace (annotation %q=%q) — refusing to proceed and risk using unrelated or empty storage", targetNamespace, targetName, i, reboundFromAnnotation, pvc.Annotations[reboundFromAnnotation])
		}
	}
	return nil
}
