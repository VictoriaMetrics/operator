...
type VLogsSpec struct {
	...
	// StorageMeta defines annotations and labels attached to PVC for given vmlogs CR
	// +optional
	...
	// ServiceSpec that will be added to vmlogs service spec
	// +optional
	ServiceSpec *AdditionalServiceSpec `json:"serviceSpec,omitempty"`
	// ServiceScrapeSpec that will be added to vmlogs VMServiceScrape spec
	// +optional
...
func (r VLogs) PrefixedName() string {
	return fmt.Sprintf("vmlogs-%s", r.Name)
}
...
func (r *VLogs) HasSpecChanges() (bool, error) {
	var prevSingleSpec VLogsSpec
	lastAppliedSingleJSON := r.Annotations["operator.victoriametrics/last-applied-spec"]
	if len(lastAppliedSingleJSON) == 0 {
		return true, nil
	}
	if err := json.Unmarshal([]byte(lastAppliedSingleJSON), &prevSingleSpec); err != nil {
		return true, fmt.Errorf("cannot parse last applied vlogs spec value: %s : %w", lastAppliedSingleJSON, err)
	}
