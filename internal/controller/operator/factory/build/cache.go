...
func (c *TLSCreds) buildAssetKey(prefix string, name string, key string) string {
	if name == "" || key == "" {
		return ""
	}
...
func (ac *AssetsCache) InjectAssetsIntoPodSpec(s *corev1.PodSpec) {
	for kind, cfg := range ac.cfg {
		if len(ac.output[kind].Data) == 0 {
			continue
		}
...
func (ac *AssetsCache) InjectAssetsIntoContainer(c *corev1.Container) {
	for kind, cfg := range ac.cfg {
		if len(ac.output[kind].Data) == 0 {
			continue
		}
...
func (ac *AssetsCache) GetOutput() map[ResourceKind]*corev1.Secret {
	...
	for kind, secret := range ac.output {
...
func (ac *AssetsCache) BuildTLSCreds(ns string, cfg *vmv1beta1.TLSConfig) (*TLSCreds, error) {
	...
	if len(cfg.CAFile) > 0 {
		...
		secret, err := ac.LoadKeyFromSecretOrConfigMap(ns, &cfg.CA)
		if err != nil {
			return nil, fmt.Errorf("cannot fetch ca: %w", err)
		}
		key := creds.buildAssetKey(ns, cfg.CA.PrefixedName(), cfg.CA.Key())
		creds.CAFile = ac.addToOutput(TLSResourceKind, key, secret)
	}

	if len(cfg.CertFile) > 0 {
		creds.CertFile = cfg.CertFile
	} else if len(cfg.Cert.PrefixedName()) > 0 {
		secret, err := ac.LoadKeyFromSecretOrConfigMap(ns, &cfg.Cert)
		if err != nil {
			return nil, fmt.Errorf("cannot fetch cert: %w", err)
		}
		key := creds.buildAssetKey(ns, cfg.Cert.PrefixedName(), cfg.Cert.Key())
		creds.CertFile = ac.addToOutput(TLSResourceKind, key, secret)
	}
	...
	if len(cfg.KeyFile) > 0 {
		...
		key := creds.buildAssetKey(ns, cfg.KeySecret.Name, cfg.KeySecret.Key)
		creds.KeyFile = ac.addToOutput(TLSResourceKind, key, secret)
		creds.Key = secret
	}
	return creds, nil
}
