...
1. deploys a buffering VMAgent pointed directly at the target's future write endpoint —
   ...
   redirects the release's Service (cluster charts: just the insert Service) at it, so incoming
...
**`NoDowntime`** — never touches the old workload(s) or their PVC(s), so reads stay available
for (almost) the whole window too. For cluster charts, only the insert Service is redirected to
the buffer, so reads against the select Service keep working throughout. For single-node
charts, the release has just one Service serving both reads and writes, so redirecting it to
