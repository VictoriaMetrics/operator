...
func selectStreamAggrRules(ctx context.Context, rclient client.Client, cr *vmv1beta1.VMAgent, opts *k8stools.SelectorOpts) ([]*vmv1.VMStreamAggrRule, error) {
	...
	build.OrderByKeys(selectedRules, namespacedNames)
...
func createOrUpdateStreamAggrConfig(ctx context.Context, rclient client.Client, cr, prevCR *vmv1beta1.VMAgent, childObject *vmv1.VMStreamAggrRule) error {
	...
	if cr.Spec.StreamAggrConfig != nil {
		...
		rules, err = selectStreamAggrRules(ctx, rclient, cr, opts)
		if err != nil {
	...
	for _, rw := range cr.Spec.RemoteWrite {
		...
		if rw.StreamAggrConfig != nil {
			...
			rls, err = selectStreamAggrRules(ctx, rclient, cr, opts)
			if err != nil {
				return fmt.Errorf("selecting StreamAggrRules failed: %w", err)
			}
		}
