...
// runPhases runs phases in order, stopping at (and returning) the first error.
func runPhases(phases ...func() error) error {
...
func AgentBaseMatches[RW any, Agent bufferAgentCR](existing, want Agent, existingRW, wantRW []RW) bool {
	return len(existingRW) == 1 && len(wantRW) == 1 && reflect.DeepEqual(existingRW[0], wantRW[0]) &&
		maps.Equal(existing.GetExtraArgs(), want.GetExtraArgs())
}
...
func deleteDependentConfigs(ctx context.Context, c client.Client, target client.Object, configMaps []corev1.ConfigMap, secrets []corev1.Secret) error {
	...
	list := func(l client.ObjectList, kind string) error {
		if err := c.List(ctx, l, client.InNamespace(namespace)); err != nil {
			return fmt.Errorf("cannot list %s in %s to check for shared dependent config references: %w", kind, namespace, err)
		}
		return nil
	}
	...
	if err := list(&deps, "Deployments"); err != nil {
		return err
	}
	...
	if err := list(&stss, "StatefulSets"); err != nil {
		return err
	}
	...
	if err := list(&dss, "DaemonSets"); err != nil {
		return err
	}
	...
	if err := list(&rss, "ReplicaSets"); err != nil {
		return err
	}
	...
	if err := list(&rcs, "ReplicationControllers"); err != nil {
		return err
	}
	...
	if err := list(&sas, "ServiceAccounts"); err != nil {
		return err
	}
	...
	if err := list(&ingresses, "Ingresses"); err != nil {
		return err
	}
	...
	if err := list(&cronJobs, "CronJobs"); err != nil {
		return err
	}
