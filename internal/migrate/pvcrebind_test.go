...
func TestEnsureReclaimPolicyRestored_DoesNotRestoreBeforePVCBinds(t *testing.T) {
	setPVCTimeouts(t)
	...
	pendingTargetPVC := &corev1.PersistentVolumeClaim{
	...
	scheme := runtime.NewScheme()
	utilruntime.Must(clientgoscheme.AddToScheme(scheme))
	rejectPVUpdate := interceptor.Funcs{
		Update: func(ctx context.Context, cl client.WithWatch, obj client.Object, opts ...client.UpdateOption) error {
			if _, ok := obj.(*corev1.PersistentVolume); ok {
				return fmt.Errorf("unexpected PersistentVolume Update call before PVC bound: %v", obj)
			}
			return cl.Update(ctx, obj, opts...)
		},
	}
	c := fake.NewClientBuilder().WithScheme(scheme).WithStatusSubresource(&corev1.PersistentVolumeClaim{}).
		WithInterceptorFuncs(rejectPVUpdate).
		WithObjects(pv, pendingTargetPVC).Build()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	err := ensureReclaimPolicyRestored(ctx, c, types.NamespacedName{Name: "vmsingle-newrelease", Namespace: "default"})
	require.Error(t, err)
	assert.ErrorContains(t, err, "waiting for PVC")

	var gotPV corev1.PersistentVolume
	require.NoError(t, c.Get(ctx, types.NamespacedName{Name: "pv-data"}, &gotPV))
	assert.Equal(t, corev1.PersistentVolumeReclaimRetain, gotPV.Spec.PersistentVolumeReclaimPolicy)
	assert.Contains(t, gotPV.Annotations, originalReclaimPolicyAnnotation)
}
...
func TestRebindPVC_CollidingTargetBoundToDifferentVolumeIsAnError(t *testing.T) {
	...
	collidingTarget.Status.Phase = corev1.ClaimBound
	require.NoError(t, c.Status().Update(ctx, collidingTarget))
