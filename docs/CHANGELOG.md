...
**Update note 1**: [vmoperator](https://docs.victoriametrics.com/operator/): removed `spec.useLegacyNaming`, introduced in v0.73.0, from all CRDs. Toggling it was already unsafe on its own: since it changes the naming convention for StatefulSets/Deployments/PVCs, flipping it on an existing CR causes the operator to rename/recreate those resources, and for PVCs specifically this meant binding to a *different*, empty PersistentVolumeClaim rather than migrating any existing data — a footgun the operator never covered and that would have taken significant work to handle safely (PV/PVC adoption, storage rebinding). The new `helm-converter migrate` command (see below) replaces its intended use case: it migrates by explicitly discovering and rebinding/repointing existing resources by identity, rather than relying on naming convention. Any existing CR with `useLegacyNaming: true` will have the field ignored by the operator (the CRD schema still accepts it) and revert to standard naming on the next reconcile — pin to the previous operator version until you've completed migration, or re-migrate using the new tool.
