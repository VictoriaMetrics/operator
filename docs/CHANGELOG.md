...
**Update note 2**: [vmcluster](https://docs.victoriametrics.com/operator/resources/vmcluster/), [vmalertmanager](https://docs.victoriametrics.com/operator/resources/vmalertmanager/): `serviceSpec.useAsDefault: true` combined with a `spec.type` other than `ClusterIP`, or with an explicit `spec.clusterIP`/`spec.clusterIPs` other than `None`, is now rejected at `spec.vmselect.serviceSpec` and `spec.vmstorage.serviceSpec` of `VMCluster` and at `spec.serviceSpec` of `VMAlertmanager`, since the default `Service` of these components must stay headless. Reconciliation of such an object fails until `useAsDefault` is removed from its `serviceSpec`, which turns it into an additional `Service`. A `serviceSpec` that keeps the default `Service` headless, e.g. `spec.type: ClusterIP` without `spec.clusterIP`, is still applied as before. See [#2491](https://github.com/VictoriaMetrics/operator/pull/2491).
...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/operator/resources/vmagent/), [vmsingle](https://docs.victoriametrics.com/operator/resources/vmsingle/): remove the service discovery `Role` and `RoleBinding` from the namespaces watched by the operator, when the object stops using the operator managed `ServiceAccount` and when the object is deleted. Previously, they were left behind, since Kubernetes cannot garbage-collect them without an owner reference, which a namespaced object cannot have outside of its own namespace. This bug was introduced in [v0.73.0](https://github.com/VictoriaMetrics/operator/releases/tag/v0.73.0), when the operator started creating these objects in every watched namespace. See [#2655](https://github.com/VictoriaMetrics/operator/pull/2655).
...
* BUGFIX: [vmcluster](https://docs.victoriametrics.com/operator/resources/vmcluster/), [vmalertmanager](https://docs.victoriametrics.com/operator/resources/vmalertmanager/): reject a `serviceSpec` with `useAsDefault: true` that makes the default `Service` of `vmselect`, `vmstorage` or `VMAlertmanager` non-headless, that is a `spec.type` other than `ClusterIP`, or an explicit `spec.clusterIP`/`spec.clusterIPs` other than `None`. Previously, such a `serviceSpec` was applied as is, and these components silently lost the per-pod DNS records that only a headless `Service` provides. See [#2487](https://github.com/VictoriaMetrics/operator/issues/2487).
