...
func Test_mergeServiceSpec(t *testing.T) {
	f := func(svc *corev1.Service, svcSpec *vmv1beta1.AdditionalServiceSpec, validate func(svc *corev1.Service) error) {
		t.Helper()
		additionalSvc := AdditionalServiceFromDefault(svc, svcSpec)
		if err := validate(additionalSvc); err != nil {
	...
	}, &vmv1beta1.AdditionalServiceSpec{
		Spec: corev1.ServiceSpec{
			Ports: []corev1.ServicePort{
				{Name: "metrics"},
			},
