...
* BUGFIX: [vmuser](https://docs.victoriametrics.com/operator/resources/vmuser/), [vmauth](https://docs.victoriametrics.com/operator/resources/vmauth/): fold `spec.headers`/`spec.response_headers` into each `targetRefs` entry that doesn't define its own, for `VMUser` and `VMAuth`'s `unauthorizedUserAccessSpec`. The operator builds each generated `url_map` entry directly from its `targetRefs` entry, and previously wrote `spec.headers`/`spec.response_headers` only onto the sibling user-level config key instead of into those entries; with more than one `targetRefs` entry, backends never received them, so tenant-identifying headers like `AccountID`/`ProjectID` never reached any backend. See [#2611](https://github.com/VictoriaMetrics/operator/issues/2611).
