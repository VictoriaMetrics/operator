...
// +kubebuilder:rbac:groups=operator.victoriametrics.com,resources=vmalerts/finalizers,verbs=patch
...
func (r *VMAlertReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	result, err = reconcileAndTrackStatus(ctx, r.Client, instance.DeepCopy(), r.name, func() (ctrl.Result, error) {
		maps, err := vmalert.CreateOrUpdateRuleConfigMaps(ctx, r, &instance, nil, instance.HasNotifiersConfigured())
		if err != nil {
			return result, err
		}
