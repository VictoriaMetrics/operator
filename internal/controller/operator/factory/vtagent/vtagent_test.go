...
func TestCreateOrUpdateService(t *testing.T) {
	type opts struct {
		cr                *vmv1.VTAgent
		prevCR            *vmv1.VTAgent
		validate          func(t *testing.T, rclient client.Client)
		predefinedObjects []runtime.Object
	}
	f := func(o opts) {
		t.Helper()
		fclient := k8stools.GetTestClientWithObjects(o.predefinedObjects)
		require.NoError(t, createOrUpdateService(context.TODO(), fclient, o.cr, o.prevCR))
		if o.validate != nil {
			o.validate(t, fclient)
		}
	}

	// useAsDefault mode must not create a separate additional service,
	// and must merge the override spec (type, extra ports) into the main one.
	f(opts{
		cr: &vmv1.VTAgent{
			ObjectMeta: metav1.ObjectMeta{Name: "agent", Namespace: "default"},
			Spec: vmv1.VTAgentSpec{
		...
		validate: func(t *testing.T, rclient client.Client) {
			...
			require.NoError(t, rclient.List(context.TODO(), &svcList))
			require.Len(t, svcList.Items, 1, "useAsDefault must not create a separate additional service")
			svc := svcList.Items[0]
			assert.Equal(t, "vtagent-agent", svc.Name)
			assert.Equal(t, corev1.ServiceTypeLoadBalancer, svc.Spec.Type)
			var hasHTTPPort bool
			for _, p := range svc.Spec.Ports {
				if p.Name == "http" {
					hasHTTPPort = true
				}
			}
			assert.True(t, hasHTTPPort, "expected default http port to be preserved in the merged useAsDefault service")
		},
	})

	// without useAsDefault, both the main and the additional service must exist.
	f(opts{
		cr: &vmv1.VTAgent{
			ObjectMeta: metav1.ObjectMeta{Name: "agent", Namespace: "default"},
			Spec: vmv1.VTAgentSpec{
