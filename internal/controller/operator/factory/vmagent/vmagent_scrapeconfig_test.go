...
func TestCreateOrUpdateConfigurationSecret(t *testing.T) {
	...
	f := func(opts opts) {
		...
		if opts.c != nil {
			*config.MustGetBaseConfig() = *opts.c
			defer func() {
		...
		if err := createOrUpdateConfigurationSecret(ctx, testClient, opts.cr, nil, nil, ac); err != nil {
			t.Errorf("CreateOrUpdateConfigurationSecret() error = %v", err)
		}
		var expectSecret corev1.Secret
		if err := testClient.Get(ctx, types.NamespacedName{Namespace: opts.cr.Namespace, Name: opts.cr.PrefixedName()}, &expectSecret); err != nil {
		...
		gr.Close()
		assert.Equal(t, opts.want, string(data))
	}
