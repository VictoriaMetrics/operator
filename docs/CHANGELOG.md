...
* FEATURE: [vmsingle](https://docs.victoriametrics.com/operator/resources/vmsingle/): added `spec.storageExistingClaimName` to mount an existing `PersistentVolumeClaim` as the storage data volume instead of provisioning one. Mutually exclusive with `spec.storageDataPath` and takes precedence over `spec.storage`. See [#2259](https://github.com/VictoriaMetrics/operator/pull/2259).
