...
func newPodSpec(cr *vmv1beta1.VMAlert, ruleConfigMapNames []string, ac *build.AssetsCache) (*appsv1.DeploymentSpec, error) {
	...
	for _, name := range ruleConfigMapNames {
		idx, ok := ruleBucketIndex(cr, name)
		if !ok {
			continue
		}
		volumes = append(volumes, corev1.Volume{
			Name: fmt.Sprintf("rules-src-%d", idx),
			VolumeSource: corev1.VolumeSource{
	...
	if cr.HasConfigReloader() {
		...
		for _, name := range ruleConfigMapNames {
			...
			build.AddWatchTargetDir(&crc,
				corev1.VolumeMount{
					Name:      fmt.Sprintf("rules-src-%d", idx),
					MountPath: path.Join("/etc/vmalert", fmt.Sprintf("rules-src-%d", idx)),
					ReadOnly:  true,
				},
				path.Join(vmAlertRulesOutDir, fmt.Sprintf("rules-src-%d", idx)),
			)
		}
...
func buildArgs(cr *vmv1beta1.VMAlert, ruleConfigMapNames []string, ac *build.AssetsCache) ([]string, error) {
	...
	for _, name := range ruleConfigMapNames {
		...
		args = append(args, fmt.Sprintf("-rule=%q", path.Join(vmAlertRulesOutDir, fmt.Sprintf("rules-src-%d", idx), "*.yaml")))
	}
...
func createOrUpdateVPA(ctx context.Context, rclient client.Client, cr, prevCR *vmv1beta1.VMAlert) error {
	if cr.Spec.VPA == nil {
		return nil
	}
