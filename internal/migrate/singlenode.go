...
func SingleNode[Target singleNodeCR, Agent bufferAgentCR](ctx context.Context, c client.Client, httpClient *http.Client, opts Options, target Target, engine SingleNodeEngine[Target, Agent], noDowntime bool) (err error) {
	...
	case resuming:
		...
		case !strings.HasPrefix(existingTargetPVC.Annotations[reboundFromAnnotation], opts.Namespace+"/"):
			return fmt.Errorf("no Deployment found for release %q in namespace %q, and PVC %s/%s exists but wasn't created by this tool's rebindPVC from a source PVC in this namespace (annotation %q=%q) — refusing to adopt what may be an unrelated, coincidentally-named PVC; delete it manually first if that's intended, or investigate why the old Deployment is missing", opts.ReleaseName, opts.Namespace, target.GetNamespace(), targetPVCName, reboundFromAnnotation, existingTargetPVC.Annotations[reboundFromAnnotation])
		}
	...
	oldSvc := &d.Services[0]
	switch {
	case !resuming && selectorDiffersFromBase(oldSvc.Spec.Selector, d.Deployment.Spec.Selector.MatchLabels) &&
