...
func (zs *zones) Less(i, j int) bool {
	if zs.vmclusters[i].Status.UpdateStatus == vmv1beta1.UpdateStatusFailed && zs.vmclusters[j].Status.UpdateStatus != vmv1beta1.UpdateStatusFailed {
		return true
	}
	if zs.vmclusters[j].Status.UpdateStatus == vmv1beta1.UpdateStatusFailed && zs.vmclusters[i].Status.UpdateStatus != vmv1beta1.UpdateStatusFailed {
		return false
	}
...
func getZones(ctx context.Context, rclient client.Client, cr *vmv1alpha1.VMDistributed) (*zones, error) {
	...
	for i := range cr.Spec.Zones {
		...
		vmAgentSpecData, err := json.Marshal(vmAgentCustomSpec)
		if err != nil {
			return nil, fmt.Errorf("spec.zones[%d].vmagent.spec: failed to marshal: %w", i, err)
		}
		var vmAgentSpec vmv1beta1.VMAgentSpec
		if err := json.Unmarshal(vmAgentSpecData, &vmAgentSpec); err != nil {
			return nil, fmt.Errorf("spec.zones[%d].vmagent.spec: failed to unmarshal: %w", i, err)
		}
		...
		if len(vmAgentSpec.RemoteWrite) != len(zs.vmclusters) {
			vmAgentSpec.RemoteWrite = slicesutil.SetLength(vmAgentSpec.RemoteWrite, len(zs.vmclusters))
		}
		for j := range cr.Spec.Zones {
			...
			remoteWriteData, err := json.Marshal(customRemoteWrite)
			if err != nil {
				return nil, fmt.Errorf("spec.zones[%d].vmagent: failed to marshal remote write for zone[%d]: %w", i, j, err)
			}
			remoteWrite := &vmAgentSpec.RemoteWrite[j]
			if err := json.Unmarshal(remoteWriteData, remoteWrite); err != nil {
				return nil, fmt.Errorf("spec.zones[%d].vmagent: failed to unmarshal remote write for zone[%d]: %w", i, j, err)
			}
...
func (zs *zones) upgrade(ctx context.Context, rclient client.Client, cr *vmv1alpha1.VMDistributed, i int) error {
	...
	if !vmCluster.CreationTimestamp.IsZero() && !vmAgent.CreationTimestamp.IsZero() {
		...
		if err := zs.waitForEmptyPQ(ctx, rclient, defaultMetricsCheckInterval, i); err != nil {
			return fmt.Errorf("zone=%s: failed to wait till VMCluster=%s queue is empty: %w", item, nsn, err)
		}

		// excluding zone from VMAuth LB
		if err := zs.updateLB(ctx, rclient, cr, i); err != nil {
			return fmt.Errorf("zone=%s: failed to update VMAuth LB with excluded VMCluster=%s: %w", item, nsn, err)
		}
	}

	// reconcile VMCluster
	if err := reconcile.VMCluster(ctx, rclient, vmCluster, nil, &owner); err != nil {
		return fmt.Errorf("zone=%s: failed to reconcile VMCluster=%s: %w", item, nsn, err)
	}

	// reconcile VMAgent
	if err := reconcile.VMAgent(ctx, rclient, vmAgent, nil, &owner); err != nil {
		return fmt.Errorf("zone=%s: failed to reconcile VMAgent=%s: %w", item, nsn, err)
	}
