...
func TestNoDowntimeSingleNode(t *testing.T) {
	...
	target := &vmv1.VLSingle{
		...
		Spec:       vmv1.VLSingleSpec{Storage: &corev1.PersistentVolumeClaimSpec{}},
	}
	...
	go simulateVLAgentOperational(ctx, c, types.NamespacedName{Name: target.PrefixedName() + "-migration-buffer", Namespace: ns})
...
// simulateVLAgentOperational mimics the operator's own reconcile loop marking a freshly-created
// buffer VLAgent Operational, which the fake client won't do on its own.
func simulateVLAgentOperational(ctx context.Context, c client.Client, nsn types.NamespacedName) {
	for {
		var cr vmv1.VLAgent
		if err := c.Get(ctx, nsn, &cr); err == nil {
			cr.Status.UpdateStatus = vmv1beta1.UpdateStatusOperational
