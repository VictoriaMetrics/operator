...
func VMBackupManager(
	...
	if cr.Image.PullPolicy == "" {
		cr.Image.PullPolicy = v1.PullIfNotPresent
	}
	if cr.Port == "" {
	...
	ports = append(ports, v1.ContainerPort{Name: "http", Protocol: "TCP", ContainerPort: intstr.Parse(cr.Port).IntVal})
	...
	_, mounts = license.MaybeAddToVolumes(nil, mounts, victoriametricsv1beta1.SecretsDir)
	args = license.MaybeAddToArgs(args, victoriametricsv1beta1.SecretsDir)
	...
	extraEnvs = append(extraEnvs, v1.EnvVar{
		Name: "POD_NAME",
