...
node-exporter or other node-based exporters. By default, `__address__` is set at kubernetes internalIP - first IP address of node.
`VMAgent` nodeScrapeSelector must match `VMNodeScrape` lables.
