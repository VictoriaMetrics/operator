			...
			sc: &vmv1beta1.VMScrapeConfig{
				...
				Spec: vmv1beta1.VMScrapeConfigSpec{
					...
					DNSSDConfigs: []vmv1beta1.DNSSDConfig{
						{
							Names: []string{"vmsingle-0.vmsingle.default.svc.cluster.local"},
							Port:  ptr.To(8429),
						},
					},
...
dns_sd_configs:
- names:
  - vmsingle-0.vmsingle.default.svc.cluster.local
  port: 8429
ec2_sd_configs:
- region: us-west-2
  port: 9404
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := generateScrapeConfig(ctx, tt.cr, tt.sc, ac)
			if err != nil {
