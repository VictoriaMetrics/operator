...
func (c *config) override(cr *vmv1.VMAnomaly, ac *build.AssetsCache) error {
	if cr.Spec.Readers != nil && len(cr.Spec.Readers.VM) > 0 {
		...
		if err = r.ClientConfig.override(cr, &vm.VMAnomalyHTTPClientSpec, ac); err != nil {
	...
	if cr.Spec.Writers != nil && len(cr.Spec.Writers.VM) > 0 {
		...
		if err = w.ClientConfig.override(cr, &vm.VMAnomalyHTTPClientSpec, ac); err != nil {
			return fmt.Errorf("failed to update HTTP client for anomaly writer, name=%q: %w", cr.Name, err)
		}
		w.Class = "vm"
		c.Writer.validatable = &w
	}
