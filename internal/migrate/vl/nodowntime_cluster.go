...
func NoDowntimeCluster(ctx context.Context, c client.Client, httpClient *http.Client, opts migrate.Options, target *vmv1.VLCluster) error {
	...
	if err := c.Delete(ctx, aliasSvc); err != nil && !k8serrors.IsNotFound(err) {
		return migrate.RevertSelectorAndFail(ctx, c, oldInsertSvc, originalInsertSelector, fmt.Errorf("cannot delete alias Service to stop writes reaching old vlinsert before the snapshot: %w", err))
	}
	...
	if err := migrate.WaitEndpointsReady(ctx, c, target.Namespace, target.PrefixedName(vmv1beta1.ClusterComponentInsert), "http"); err != nil {
		return fmt.Errorf("target vlinsert's Service did not become ready: %w", err)
	}
	if err := migrate.CutoverServices(ctx, c, []*corev1.Service{oldInsertSvc}, target.SelectorLabels(vmv1beta1.ClusterComponentInsert)); err != nil {
		return fmt.Errorf("final vlinsert traffic cutover failed: %w", err)
	}
	...
	if err := migrate.WaitEndpointsReady(ctx, c, target.Namespace, target.PrefixedName(vmv1beta1.ClusterComponentSelect), "http"); err != nil {
		return fmt.Errorf("vlinsert already cut over to the target, but vlselect's Service did not become ready — fix its Service selector manually once it is: %w", err)
	}
