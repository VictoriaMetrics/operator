...
func TestSelectRules(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlert, want map[string]string, predefinedObjects []runtime.Object) {
		t.Helper()
		ctx := context.Background()
		fclient := k8stools.GetTestClientWithObjects(predefinedObjects)
		got, _, err := selectRulesContent(ctx, fclient, cr)
		if err != nil {
			t.Errorf("SelectRules() error = %v", err)
			return
		}
		for ruleName, content := range got {
			if !assert.Equal(t, want[ruleName], content) {
				t.Errorf("SelectRules() got = %v, want %v", content, want[ruleName])
			}
		}
	}

	// select default rule
	f(&vmv1beta1.VMAlert{}, map[string]string{
...
`}, []runtime.Object{
	...
	}, map[string]string{"monitoring-error-alert-at-monitoring.yaml": `groups:
- name: error-alert
  interval: 10s
  rules:
  - alert: alerting-2
...
`}, []runtime.Object{
	...
	}, map[string]string{"default-vmalert.yaml": `
groups:
- name: vmAlertGroup
  rules:
     - alert: error writing to remote
...
func TestCreateOrUpdateRuleConfigMaps(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlert, want []string, predefinedObjects []runtime.Object) {
		t.Helper()
		fclient := k8stools.GetTestClientWithObjects(predefinedObjects)
		got, err := CreateOrUpdateRuleConfigMaps(context.TODO(), fclient, cr, nil)
		if err != nil {
			t.Errorf("CreateOrUpdateRuleConfigMaps() error = %v", err)
			return
		}
		if !reflect.DeepEqual(got, want) {
			t.Errorf("CreateOrUpdateRuleConfigMaps() got = %v, want %v", got, want)
		}
	}
	...
	}, []string{"vm-base-vmalert-rulefiles-0"}, nil)
}
...
func Test_deduplicateRules(t *testing.T) {
	f := func(origin, want []*vmv1beta1.VMRule) {
		...
		if len(diff) > 0 {
			t.Errorf("deduplicateRules() %v = %v, want %v", diff, got, want)
		}
	}
...
func Test_rulesCMDiff(t *testing.T) {
	f := func(currentCMs, newCMs, toCreate, toUpdate []corev1.ConfigMap) {
		got, got1 := rulesCMDiff(currentCMs, newCMs)
		assert.Equal(t, toCreate, got)
		assert.Equal(t, toUpdate, got1)
	}

	// create one new
	f(nil, []corev1.ConfigMap{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name: "rules-cm-1",
			},
