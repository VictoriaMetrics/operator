...
* FEATURE: [vmcluster](https://docs.victoriametrics.com/operator/resources/vmcluster/): add `spec.globalReplicationFactor` and per-pool `spec.pools[].replicationFactor`, rendered as `vmselect`'s `-globalReplicationFactor` and per-storage-group `-replicationFactor=<pool>:<replicationFactor>` flags, completing the storage-group support introduced by `spec.pools`.
