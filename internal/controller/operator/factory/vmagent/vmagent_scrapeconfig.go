...
const (
	...
	k8sSDRoleHTTPRoute     = "httproute"
	k8sSDRoleGRPCRoute     = "grpcroute"
	k8sSDRoleNode          = "node"
)
...
func addAttachMetadata(dst yaml.MapSlice, am *vmv1beta1.AttachMetadata, role string) yaml.MapSlice {
	...
	if am.Namespace != nil && *am.Namespace {
		switch role {
		case k8sSDRolePod, k8sSDRoleService, k8sSDRoleEndpoints, k8sSDRoleEndpointslice, k8sSDRoleIngress, k8sSDRoleHTTPRoute, k8sSDRoleGRPCRoute:
			items = append(items, yaml.MapItem{
				Key:   "namespace",
				Value: true,
			})
...
func generateK8SSDConfig(ac *build.AssetsCache, opts ...generateK8SSDConfigOptions) (yaml.MapSlice, error) {
	...
	for _, o := range opts {
		...
		case o.shouldAddSelectors && !isEmptySelectors:
			...
			if o.role == k8sSDRoleEndpoints || o.role == k8sSDRoleEndpointslice {
