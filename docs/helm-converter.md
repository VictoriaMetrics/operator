...
For `victoria-metrics-alert`, the chart's legacy value aliases are resolved the same way the chart does before conversion: `server.remote.read`/`server.remote.write` merge into `server.remoteRead`/`server.remoteWrite`, `server.notifier.alertmanager` into the notifier itself, the nested `basicAuth`/`bearer` keys into the flat `basicAuth.username`/`bearerToken` ones, and a list-valued `server.notifier.url` fans out into `server.notifiers`. A set legacy value takes precedence. Plaintext credentials are moved into generated `Secret` manifests referenced from the CR, and the top-level `license` block becomes `spec.license`. A notifier without a URL is omitted, since the chart configures none either (operator releases before v0.63.0 reject a `VMAlert` with no notifier at all, so add one or `notifier.blackhole` there); if the chart's bundled `alertmanager` is enabled the conversion fails instead, because that sub-chart is not converted and a `VMAlertmanager` CR plus `spec.notifier` must be created manually.
