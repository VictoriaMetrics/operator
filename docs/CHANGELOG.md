...
**Update note 1**: [vmoperator](https://docs.victoriametrics.com/operator/): removed `spec.useLegacyNaming`, introduced in v0.73.0, from all CRDs — it was already unsafe to toggle on an existing CR, since it could bind to a different, empty PersistentVolumeClaim instead of your existing data. Any CR with `useLegacyNaming: true` will have the field ignored on the next reconcile and revert to standard naming. If you rely on it, migrate to operator-managed naming first using the new `helm-converter migrate` command (see below), or pin to the previous operator version until you have migrated.
...
* FEATURE: [helm-converter](https://docs.victoriametrics.com/operator/helm-converter/): add a `migrate` command that automates cutover from a standalone Helm-chart-deployed installation to an operator-managed CR (`victoria-metrics-single`, `victoria-logs-single`, `victoria-metrics-cluster`, `victoria-logs-cluster`), with `WithDowntime` and `NoDowntime` strategies. `WithDowntime` briefly interrupts availability during cutover, while `NoDowntime` keeps the old installation serving traffic throughout migration and leaves it untouched afterward for you to decommission whenever you're ready. See [#2511](https://github.com/VictoriaMetrics/operator/pull/2511).
