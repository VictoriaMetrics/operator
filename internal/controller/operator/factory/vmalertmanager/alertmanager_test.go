...
func TestCreateOrUpdateAlertManager(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlertmanager, validate func(set *appsv1.StatefulSet) error, predefinedObjects []runtime.Object) {
		t.Helper()
		...
		go func() {
			...
			for {
				...
				case <-tc.C:
					...
					if err := fclient.Get(ctx, types.NamespacedName{Namespace: cr.Namespace, Name: cr.PrefixedName()}, &got); err != nil {
					...
					got.Status.ReadyReplicas = *cr.Spec.ReplicaCount
					got.Status.UpdatedReplicas = *cr.Spec.ReplicaCount
		...
		err := CreateOrUpdateAlertManager(ctx, cr, fclient)
		if err != nil {
			t.Fatalf("CreateOrUpdateAlertManager() error = %v", err)
		}
	...
	}, func(set *appsv1.StatefulSet) error {
...
func Test_createDefaultAMConfig(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlertmanager, wantErr, secretMustBeMissing bool, predefinedObjects []runtime.Object) {
		t.Helper()
		fclient := k8stools.GetTestClientWithObjects(predefinedObjects)
		ctx := context.TODO()
		if err := CreateOrUpdateConfig(ctx, fclient, cr, nil); (err != nil) != wantErr {
			t.Fatalf("createDefaultAMConfig() error = %v, wantErr %v", err, wantErr)
		}
		if wantErr {
			return
		}
		...
		if err != nil {
			if k8serrors.IsNotFound(err) && secretMustBeMissing {
				return
			}
