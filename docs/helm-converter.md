...
every write made over a *new* connection through a VMAgent (VLAgent for VictoriaLogs, VTAgent
for VictoriaTraces) before touching anything, so those are never lost under either strategy — the
main thing that differs
...
For cluster charts (`victoria-metrics-cluster`, `victoria-logs-cluster`,
`victoria-traces-cluster`), the same steps run once per component
(`vmstorage`/`vmselect`/`vminsert`, `vlstorage`/`vlselect`/`vlinsert`, or
`vtstorage`/`vtselect`/`vtinsert`), discovered via the chart's own
`app.kubernetes.io/component` label: each component's old StatefulSet/Deployment is deleted,
each StatefulSet component's PVCs are rebound one per ordinal (`vmstorage`/`vmselect`, or
`vlstorage`/`vtstorage` — the insert/select-without-cache components have no persistent
storage), then the single target VMCluster/VLCluster/VTCluster CR is created once and every
component's Service is repointed after it becomes ready — insert's Service comes back from
the buffer agent, the rest onto the target's new pods.
...
separate component. The alias is a stopgap, not a seamless read path: it's named
`<prefixed-target-name>-migration-reads` (also printed in the plan output), so read clients still
need to be manually repointed at it — and then off it again before the final cutover (step 5).
Once the release's own Service starts serving both reads and writes again, `migrate` waits a
short grace period and then either deletes the alias Service automatically (with `-yes`) or asks
for confirmation; declining leaves it running for you to remove manually once every read client
has moved off it. The
`victoria-metrics-single`/`victoria-logs-single`/`victoria-traces-single`
  ...
  - apiVersion: v1
    ...
    spec:
      selector: # copy this from: kubectl get deploy -l app.kubernetes.io/instance=my-release -o jsonpath='{.items[0].spec.selector.matchLabels}'
        app.kubernetes.io/name: victoria-metrics-single # or victoria-logs-single/victoria-traces-single, matching the installed chart
...
1. deploys a buffering VMAgent (VLAgent for VictoriaLogs, VTAgent for VictoriaTraces) pointed
   directly at the target's future write endpoint — deterministic from the target's name, so
   this works before the target CR even exists. Its persistent queue absorbs every write from
   step 2 onward until
...
2. redirects the release's Service to the buffer agent so incoming writes keep flowing —
   for single-node charts, also creates the `-migration-reads` alias Service described above
   first, so reads have somewhere to go once the release's Service stops serving them,
...
For cluster charts (`victoria-metrics-cluster`, `victoria-logs-cluster`,
`victoria-traces-cluster`), only the insert component (`vminsert`/`vlinsert`/`vtinsert`, the
write path) goes through the buffer-and-cutover dance above. The select component
(`vmselect`/`vlselect`/`vtselect`, reads) has nothing to buffer, so its Service is cut over
directly once the target is ready. The storage component (`vmstorage`/`vlstorage`/`vtstorage`)
has no client-facing Service at all — insert/select address it directly — so its per-ordinal
PVCs are just force-merged (per pod, since each holds an independent shard) and
CSI-snapshotted, with no Service involved.
...
Currently supports `victoria-metrics-single`, `victoria-logs-single`, `victoria-traces-single`,
`victoria-metrics-cluster`, `victoria-logs-cluster`, and `victoria-traces-cluster` for both
`WithDowntime` and `NoDowntime`.
