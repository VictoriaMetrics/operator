...
func newErrRecreate(ctx context.Context, kind string, r client.Object) *errRecreate {
	finalizers := strings.Join(r.GetFinalizers(), ",")
	msg := fmt.Sprintf("waiting for %s=%s/%s (finalizers=[%s]) to be removed", kind, r.GetNamespace(), r.GetName(), finalizers)
