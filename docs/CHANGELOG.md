...
* FEATURE: [vmoperator](https://docs.victoriametrics.com/operator/): add `VM_OPENSHIFT_COMPATIBILITY` env var (`OpenshiftCompatibilityMode` config field, default `auto`) to enable OpenShift-specific compatibility features for managed pods. In `auto` mode the operator detects OpenShift at startup via the `security.openshift.io/v1` API group. Currently covers automatic mounting of the `openshift-service-ca.crt` ConfigMap into `VMAgent` and `VMSingle` pods at `/etc/ssl/certs/openshift-service-ca/service-ca.crt`, allowing scrape targets signed with the OpenShift service signing CA to be verified without per-target TLS config. Supported values: `auto`, `enabled`, `disabled`. See [#2971](https://github.com/VictoriaMetrics/helm-charts/issues/2971).
