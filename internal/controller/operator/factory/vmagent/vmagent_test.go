...
func TestCreateOrUpdate(t *testing.T) {
	...
	f := func(opts opts) {
		...
		ctx := context.TODO()
		if opts.mustAddPrevSpec {
			jsonSpec, err := json.Marshal(opts.cr.Spec)
			if err != nil {
				t.Fatalf("cannot set last applied spec: %s", err)
			}
			if opts.cr.Annotations == nil {
				opts.cr.Annotations = make(map[string]string)
			}
			opts.cr.Annotations["operator.victoriametrics/last-applied-spec"] = string(jsonSpec)
		}
		...
		if opts.statefulSetMode {
			if opts.cr.Spec.ShardCount != nil {
				for i := 0; i < *opts.cr.Spec.ShardCount; i++ {
					err := wait.PollUntilContextTimeout(context.Background(), 20*time.Millisecond, time.Second, false, func(ctx context.Context) (done bool, err error) {
						...
						if err := fclient.Get(ctx, types.NamespacedName{
							Namespace: "default",
							Name:      fmt.Sprintf("vmagent-%s-%d", opts.cr.Name, i),
						}, &sts); err != nil {

							return false, nil
						}
						sts.Status.ObservedGeneration = sts.Generation
						sts.Status.ReadyReplicas = ptr.Deref(opts.cr.Spec.ReplicaCount, 0)
						sts.Status.UpdatedReplicas = ptr.Deref(opts.cr.Spec.ReplicaCount, 0)
						sts.Status.CurrentReplicas = ptr.Deref(opts.cr.Spec.ReplicaCount, 0)
				...
				err := wait.PollUntilContextTimeout(context.Background(), 20*time.Millisecond, time.Second, false, func(ctx context.Context) (done bool, err error) {
					var sts appsv1.StatefulSet
					if err := fclient.Get(ctx, types.NamespacedName{Namespace: "default", Name: fmt.Sprintf("vmagent-%s", opts.cr.Name)}, &sts); err != nil {
						return false, nil
					}
					sts.Status.ReadyReplicas = ptr.Deref(opts.cr.Spec.ReplicaCount, 0)
					sts.Status.UpdatedReplicas = ptr.Deref(opts.cr.Spec.ReplicaCount, 0)
		...
		if opts.statefulSetMode && opts.cr.Spec.ShardCount == nil {
			var got appsv1.StatefulSet
			if err := fclient.Get(context.Background(), types.NamespacedName{Namespace: opts.cr.Namespace, Name: opts.cr.PrefixedName()}, &got); (err != nil) != opts.wantErr {
				t.Fatalf("CreateOrUpdate() error = %v, wantErr %v", err, opts.wantErr)
			}
...
`))},
	...
	o = opts{
		...
		predefinedObjects: []runtime.Object{k8stools.NewReadyDeployment("vmagent-example-agent", "default")},
	}
	f(o)
...
func TestBuildRemoteWriteArgs(t *testing.T) {
	...
	f := func(opts opts) {
		...
		sort.Strings(opts.want)
		got, err := buildRemoteWriteArgs(opts.cr, ac)
		if err != nil {
...
func TestCreateOrUpdateService(t *testing.T) {
	type opts struct {
		cr                *vmv1beta1.VMAgent
		wantSvc           func(svc *corev1.Service) error
		wantExtraSvc      func(svc *corev1.Service) error
		predefinedObjects []runtime.Object
	}
	f := func(opts opts) {
		t.Helper()
		...
		if err := opts.wantSvc(got); err != nil {
			t.Errorf("CreateOrUpdateService() unexpected error: %v", err)
		}
		if opts.wantExtraSvc != nil {
			var additionalSvc corev1.Service
			if err := cl.Get(ctx, types.NamespacedName{Namespace: opts.cr.Namespace, Name: opts.cr.Spec.ServiceSpec.NameOrDefault(opts.cr.Name)}, &additionalSvc); err != nil {
				t.Fatalf("unexpected error: %s", err)
			}
			if err := opts.wantExtraSvc(&additionalSvc); err != nil {
...
func TestCreateOrUpdateStreamAggrConfig(t *testing.T) {
	...
	f := func(opts opts) {
		...
		ctx := context.TODO()
		if err := createOrUpdateStreamAggrConfig(ctx, cl, opts.cr, nil); err != nil {
			t.Fatalf("CreateOrUpdateStreamAggrConfig() error = %v", err)
		}
		var createdCM corev1.ConfigMap
		if err := cl.Get(ctx,
			types.NamespacedName{
				Namespace: opts.cr.Namespace,
				Name:      build.ResourceName(build.StreamAggrConfigResourceKind, opts.cr),
			}, &createdCM,
		); err != nil {
			t.Fatalf("cannot fetch created cm: %v", err)
		}
		if err := opts.validate(&createdCM); err != nil {
			t.Fatalf("cannot validate created cm: %v", err)
		}
	}
...
func Test_buildConfigReloaderArgs(t *testing.T) {
	...
	f := func(opts opts) {
		...
		got := buildConfigReloaderArgs(opts.cr, exvms)
		sort.Strings(got)
		sort.Strings(opts.want)
		assert.Equal(t, opts.want, got)
	}
...
func TestMakeSpecForAgentOk(t *testing.T) {
	...
	f := func(opts opts) {
		...
		if err := yaml.Unmarshal([]byte(opts.want), &wantSpec); err != nil {
			t.Fatalf("not expected want: %q: \n%q", opts.want, err)
		}
		wantYAMLForCompare, err := yaml.Marshal(wantSpec)
		if err != nil {
			t.Fatalf("BUG: cannot parse as yaml: %q", err)
		}
		got, err := makeSpec(opts.cr, ac)
		if err != nil {
