...
func (r *PromServiceMonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	cr := converter.ServiceMonitor(instance, r.BaseConf)
	var owner *metav1.OwnerReference
	if len(cr.OwnerReferences) > 0 {
		owner = &cr.OwnerReferences[0]
	}

	if err = reconcile.VMServiceScrape(ctx, r.Client, cr, nil, owner, true); err != nil {
		l.Error(err, "failed to reconcile VMServiceScrape from ServiceMonitor")
	}
	return
}
...
func (*PromServiceMonitorReconciler) IsDisabled(cfg *config.BaseOperatorConf, disabledControllers sets.Set[string]) bool {
	return disabledControllers.HasAll("VMAgent", "VMSingle") || disabledControllers.Has("VMServiceScrape") || !cfg.EnabledPrometheusConverter.ServiceMonitor
}
