...
func TestSelectorMatchesBase(t *testing.T) {
	podLabels := map[string]string{"app.kubernetes.io/name": "vmstorage", "app.kubernetes.io/instance": "myrelease"}

	// A Service selector routinely omits labels its target pods also carry.
	assert.True(t, selectorMatchesBase(map[string]string{"app.kubernetes.io/name": "vmstorage"}, podLabels))
	assert.True(t, selectorMatchesBase(podLabels, podLabels))
	assert.False(t, selectorMatchesBase(nil, podLabels))
	assert.False(t, selectorMatchesBase(map[string]string{}, podLabels))
	assert.False(t, selectorMatchesBase(map[string]string{"app.kubernetes.io/name": "vmselect"}, podLabels))
	assert.False(t, selectorMatchesBase(map[string]string{"unrelated-key": "vmstorage"}, podLabels))
}
...
func TestDeleteDependentConfigs_ReferencedByCSINodePublishSecretSurvives(t *testing.T) {
	scheme := runtime.NewScheme()
	utilruntime.Must(clientgoscheme.AddToScheme(scheme))
	utilruntime.Must(vmv1beta1.AddToScheme(scheme))

	target := &vmv1beta1.VMSingle{ObjectMeta: metav1.ObjectMeta{Name: "myrelease", Namespace: "default"}}
	secret := corev1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "csi-creds", Namespace: "default"}}
	unrelatedDep := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{Name: "unrelated-app", Namespace: "default"},
		Spec: appsv1.DeploymentSpec{
			Template: corev1.PodTemplateSpec{
	...
	var check corev1.Secret
	assert.NoError(t, c.Get(ctx, types.NamespacedName{Name: "csi-creds", Namespace: "default"}, &check))
}
