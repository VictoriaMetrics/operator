...
func testNoDowntimeSingleNode(t *testing.T) {
	...
	go migratetest.SimulateAgentEndpoints(ctx, t, c, types.NamespacedName{Name: target.PrefixedName() + "-migration-buffer", Namespace: ns}, adminAddr, 9428, func() *vmv1.VLAgent { return &vmv1.VLAgent{} })
...
func TestSingleNodeEngine_AgentMatches_ComparesTransformedURL(t *testing.T) {
	...
	target := &vmv1.VLSingle{ObjectMeta: metav1.ObjectMeta{Name: "vlsingle-x", Namespace: "default"}}
	writeURL := engine.BuildAgentWriteURL(target, "http://vlsingle-x.default.svc:9428/insert/native")
	...
	stillUntransformed.Spec.RemoteWrite[0].URL = "http://vlsingle-x.default.svc:9428/insert/native"
...
func TestSingleNodeEngine_HasStorage_StorageDataPathDisablesSpecStorage(t *testing.T) {
	...
	engine := singleNodeEngine()
	assert.False(t, engine.HasStorage(target), "storageDataPath disables spec.storage — VLSingle never mounts the migrated PVC in that mode")
}
