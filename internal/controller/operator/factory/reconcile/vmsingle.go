...
func VMSingle(ctx context.Context, rclient client.Client, newObj, prevObj *vmv1beta1.VMSingle, owner *metav1.OwnerReference) error {
	...
	err := retryOnConflict(func() error {
		var existingObj vmv1beta1.VMSingle
		if err := rclient.Get(ctx, nsn, &existingObj); err != nil {
			if k8serrors.IsNotFound(err) {
				logger.WithContext(ctx).Info(fmt.Sprintf("creating new VMSingle=%s", nsn.String()))
				if err := rclient.Create(ctx, newObj); err != nil {
					return fmt.Errorf("cannot create new VMSingle=%s: %w", nsn.String(), err)
				}
				generation = newObj.Generation
				return nil
			}
			return fmt.Errorf("cannot get VMSingle=%s: %w", nsn.String(), err)
		}
		...
		logger.WithContext(ctx).Info(fmt.Sprintf("updating VMSingle %s", strings.Join(logMessageMetadata, ", ")), "spec_diff", specDiff)
		if err := rclient.Update(ctx, &existingObj); err != nil {
			return fmt.Errorf("cannot update VMSingle=%s: %w", nsn.String(), err)
		}
	...
	if err := waitForStatus(ctx, rclient, newObj, vmWaitReadyInterval, vmv1beta1.UpdateStatusOperational, generation); err != nil {
		return fmt.Errorf("failed to wait for VMSingle=%s to be ready: %w", nsn.String(), err)
	}
	return nil
}
