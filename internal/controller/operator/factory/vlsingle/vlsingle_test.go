...
func TestCreateOrUpdateService_ServiceSpecs(t *testing.T) {
	f := func(cr *vmv1.VLSingle, validate func(ctx context.Context, rclient client.Client, cr *vmv1.VLSingle)) {
		t.Helper()
		fclient := k8stools.GetTestClientWithObjects(nil)
		build.AddDefaults(fclient.Scheme())
		ctx := context.TODO()
	...
	f(&vmv1.VLSingle{
		ObjectMeta: metav1.ObjectMeta{Name: "logs-1", Namespace: "default"},
		Spec: vmv1.VLSingleSpec{
			ServiceSpecs: map[string]vmv1beta1.AdditionalServiceSpec{
				"default": {Spec: corev1.ServiceSpec{Type: corev1.ServiceTypeNodePort}},
			},
		},
	}, func(ctx context.Context, rclient client.Client, cr *vmv1.VLSingle) {
