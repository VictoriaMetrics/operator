...
**Update note 3**: [vmcluster](https://docs.victoriametrics.com/operator/resources/vmcluster/), [vmalertmanager](https://docs.victoriametrics.com/operator/resources/vmalertmanager/), [vmagent](https://docs.victoriametrics.com/operator/resources/vmagent/): `spec.vmselect.serviceSpec` of `VMCluster`, `spec.serviceSpec` of `VMAlertmanager` and `spec.serviceSpec` of a `VMAgent` with `spec.statefulMode`, with `useAsDefault: true` and an explicit `spec.type`, but no `spec.clusterIP`, now produce a `Service` with a virtual IP instead of a headless one. That `Service` is recreated during the upgrade and gets a new `clusterIP`. For a `VMCluster` with `spec.requestsLoadBalancer.enabled`, the recreated `Service` is the `vmauth` proxy of `vmselect`, while its internal `Service` stays headless. Add `spec.clusterIP: None`, or remove `spec.type`, to keep the previous behaviour. See [#2487](https://github.com/VictoriaMetrics/operator/issues/2487).
...
* FEATURE: [vmcluster](https://docs.victoriametrics.com/operator/resources/vmcluster/), [vmalertmanager](https://docs.victoriametrics.com/operator/resources/vmalertmanager/), [vmagent](https://docs.victoriametrics.com/operator/resources/vmagent/): allow a `serviceSpec` with `useAsDefault: true` to replace the headless default `Service` of `vmselect`, `VMAlertmanager` and `VMAgent` with `spec.statefulMode` with a regular one by defining `spec.type`. Previously, `spec.type: ClusterIP` still inherited `clusterIP: None` from the default `Service`, so the only way to get a virtual IP was to pin `spec.clusterIP` by hand. See [#2487](https://github.com/VictoriaMetrics/operator/issues/2487).
