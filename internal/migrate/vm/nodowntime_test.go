...
func TestNoDowntimeSingleNode(t *testing.T) {
	...
	// the buffer agent's initial write target must reference the alias Service, not oldSvc
	// directly — oldSvc's own selector gets redirected to the buffer agent moments later, so
	// a URL built from oldSvc's name would otherwise resolve back to the agent itself.
	select {
