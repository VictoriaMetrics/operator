...
func TestDeepMerge(t *testing.T) {
	...
	f := func(oss ...opts) {
		...
		for _, o := range oss {
			o.validate(base, merged, MergeDeep(merged, o.override, o.reverse))
		}
	}
	...
	// reverse=true (how pool overrides are merged onto the cluster-level base): the struct
	// being merged into already has its own fields populated and wins; the "override" argument
	// only supplies defaults for fields the first struct leaves absent.
	f(opts{
		override: &VMClusterSpec{
			VMClusterSpecBase: VMClusterSpecBase{
				ClusterVersion:     "v-defaults",
				ServiceAccountName: "sa-defaults",
				RetentionPeriod:    "90d",
				VMSelect: &VMSelect{
					CommonAppsParams: CommonAppsParams{
						ReplicaCount: ptr.To(int32(9)),
						ExtraArgs:    map[string]string{"from-defaults": "yes"},
					},
				},
			},
		},
		...
		validate: func(_, merged *VMClusterSpec, err error) {
			assert.NoError(t, err)
			assert.Equal(t, "v1.0.0", merged.ClusterVersion)
			assert.Equal(t, "base", merged.ServiceAccountName)
			assert.Equal(t, "30d", merged.RetentionPeriod)
			if !assert.NotNil(t, merged.VMSelect) || !assert.NotNil(t, merged.VMSelect.ReplicaCount) {
				return
			}
			assert.Equal(t, int32(1), *merged.VMSelect.ReplicaCount)
			assert.Equal(t, "x", merged.VMSelect.ExtraArgs["keep"])
			assert.Equal(t, "yes", merged.VMSelect.ExtraArgs["from-defaults"])
		},
	})
}
...
func TestDeepMerge_NilBase(t *testing.T) {
	var base *VMSelect
	override := &VMSelect{
		CommonAppsParams: CommonAppsParams{ReplicaCount: ptr.To(int32(2))},
	}
	assert.NotPanics(t, func() {
		assert.Error(t, MergeDeep(base, override, false))
	})
}
