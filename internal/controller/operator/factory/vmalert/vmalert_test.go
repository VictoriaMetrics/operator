...
func TestCreateOrUpdate(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlert, validator func(*appsv1.Deployment, *corev1.Secret) error, predefinedObjects []runtime.Object) {
		t.Helper()
		ctx := context.TODO()
		fclient := k8stools.GetTestClientWithObjects(predefinedObjects)
		err := CreateOrUpdate(ctx, cr, fclient, nil)
		if err != nil {
		...
		if validator != nil {
			...
			if err := fclient.Get(ctx, types.NamespacedName{Namespace: cr.Namespace, Name: cr.PrefixedName()}, &generatedDeploment); err != nil {
				t.Fatalf("cannot find generated deployment=%q, err: %v", cr.PrefixedName(), err)
			}
			var generatedTLSSecret corev1.Secret
			tlsSecretName := build.ResourceName(build.TLSAssetsResourceKind, cr)
			if err := fclient.Get(ctx, types.NamespacedName{Namespace: cr.Namespace, Name: tlsSecretName}, &generatedTLSSecret); err != nil {
				if !k8serrors.IsNotFound(err) {
					t.Fatalf("unexpected error during attempt to get tls secret=%q, err: %v", build.ResourceName(build.TLSAssetsResourceKind, cr), err)
				}
			}
			if err := validator(&generatedDeploment, &generatedTLSSecret); err != nil {
	...
	}, func(d *appsv1.Deployment, s *corev1.Secret) error {
		var foundOk bool
...
func TestBuildNotifiers(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlert, want []string, predefinedObjects []runtime.Object) {
		t.Helper()
		ctx := context.Background()
		fclient := k8stools.GetTestClientWithObjects(predefinedObjects)
		ac := getAssetsCache(ctx, fclient, cr)
		if got, err := buildNotifiersArgs(cr, ac); err != nil {
	...
	}, []string{"-notifier.config=" + notifierConfigMountPath + "/cfg.yaml"}, nil)
...
func TestCreateOrUpdateService(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlert, want func(svc *corev1.Service) error) {
		t.Helper()
		ctx := context.TODO()
...
func Test_buildVMAlertArgs(t *testing.T) {
	f := func(cr *vmv1beta1.VMAlert, cmNames, want []string) {
		t.Helper()
		ctx := context.Background()
		fclient := k8stools.GetTestClientWithObjects(nil)
		ac := getAssetsCache(ctx, fclient, cr)
		if got, err := buildArgs(cr, cmNames, ac); err != nil {
