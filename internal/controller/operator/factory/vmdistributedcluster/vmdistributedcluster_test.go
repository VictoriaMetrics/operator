...
func TestReconcileInlineVMCluster(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = vmv1beta1.AddToScheme(scheme)
	_ = vmv1alpha1.AddToScheme(scheme)
	...
	testCases := []struct {
		...
		index                int
		refOrSpec            vmv1alpha1.VMClusterRefOrSpec
		initialObjects       []client.Object // For trackingClient setup
		rClient              client.Client   // Custom client if needed (e.g., customErrorClient)
		expectedVMCluster    *vmv1beta1.VMCluster
		expectedActions      []action
		expectedErrSubstring string
	}{
		{
			name:      "should create a new VMCluster if not found",
			crName:    "test-dc",
			namespace: "default",
			index:     0,
			refOrSpec: vmv1alpha1.VMClusterRefOrSpec{
		...
		{
			name:      "should update existing VMCluster if spec differs",
			crName:    "test-dc",
			namespace: "default",
			index:     0,
			refOrSpec: vmv1alpha1.VMClusterRefOrSpec{
		...
		{
			name:      "should not update if VMCluster spec is identical",
			crName:    "test-dc",
			namespace: "default",
			index:     0,
			refOrSpec: vmv1alpha1.VMClusterRefOrSpec{
		...
		},
		{
			...
			expectedVMCluster:    nil,
			expectedActions:      []action{},
			expectedErrSubstring: "failed to get VMCluster",
		},
	}
