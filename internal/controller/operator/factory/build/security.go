...
func WarnOpenShiftSecurityContext(sc *vmv1beta1.SecurityContext) []string {
	...
	if psc.RunAsUser != nil {
		warnings = append(warnings, "spec.securityContext.runAsUser is explicitly set; on OpenShift this value must fall within the namespace-allocated UID range or pod admission will fail — consider removing it and letting the SCC assign the UID")
	}
	if psc.RunAsGroup != nil {
		warnings = append(warnings, "spec.securityContext.runAsGroup is explicitly set; on OpenShift this value must fall within the namespace-allocated GID range or pod admission will fail — consider removing it and letting the SCC assign the GID")
	}
	if psc.FSGroup != nil {
		warnings = append(warnings, "spec.securityContext.fsGroup is explicitly set; on OpenShift this value must fall within the namespace-allocated GID range or pod admission will fail — consider removing it and letting the SCC assign the fsGroup")
	}
	return warnings
}
