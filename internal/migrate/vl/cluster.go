...
func clusterEngine() migrate.ClusterEngine[*vmv1.VLCluster, *vmv1.VLAgent] {
	return migrate.ClusterEngine[*vmv1.VLCluster, *vmv1.VLAgent]{
		...
		BuildAgentWriteURL: func(target *vmv1.VLCluster, remoteWriteURL string) string {
			var extraArgs map[string]string
			if target.Spec.VLInsert != nil {
				extraArgs = target.Spec.VLInsert.ExtraArgs
			}
			return bufferAgentWriteURL(remoteWriteURL, normalizePathPrefix(extraArgs["http.pathPrefix"]))
		},
		...
		StorageUsesHPA: func(target *vmv1.VLCluster) bool {
			return target.Spec.VLStorage != nil && target.Spec.VLStorage.ReplicaCount == nil && target.Spec.VLStorage.HPA != nil
		},
		StorageMaxReplicas: func(target *vmv1.VLCluster) int32 {
			if target.Spec.VLStorage == nil || target.Spec.VLStorage.HPA == nil {
				return 0
			}
			return target.Spec.VLStorage.HPA.MaxReplicas
		},
	}
}
