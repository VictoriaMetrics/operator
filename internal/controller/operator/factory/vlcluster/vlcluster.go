...
func deletePrevStateResources(ctx context.Context, rclient client.Client, cr, prevCR *vmv1.VLCluster) error {
	...
	prevStorage := prevSpec.VLStorage
	prevSelect := prevSpec.VLSelect
	prevInsert := prevSpec.VLInsert
	...
	if prevStorage != nil {
		if newStorage == nil {
			if err := finalize.OnVLStorageDelete(ctx, rclient, cr, prevStorage); err != nil {
	...
	if prevSelect != nil {
		if newSelect == nil {
			if err := finalize.OnVLSelectDelete(ctx, rclient, cr, prevSelect); err != nil {
	...
	if prevInsert != nil {
		if newInsert == nil {
			if err := finalize.OnVLInsertDelete(ctx, rclient, cr, prevInsert); err != nil {
