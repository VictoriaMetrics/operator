...
func createOrUpdateVPA(ctx context.Context, rclient client.Client, cr, prevCR *vmv1.VLSingle) error {
	if cr.Spec.VPA == nil {
		return nil
	}
...
func createOrUpdateService(ctx context.Context, rclient client.Client, cr, prevCR *vmv1.VLSingle) error {
	...
	for key, spec := range vmv1beta1.ResolveExtraServiceSpecs(cr.Spec.ServiceSpec, cr.Spec.ServiceSpecs) {
		...
		if key != "" && spec.Spec.Ports == nil && !build.FilterServicePorts(additionalSvc, key) {
			return fmt.Errorf("vlsingle additional service key %q does not match any port currently exposed by this service", key)
		}
