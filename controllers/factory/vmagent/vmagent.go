...
func buildRemoteWrites(cr *victoriametricsv1beta1.VMAgent, ssCache *scrapesSecretsCache) []string {
	...
	for i := range remoteTargets {
		...
		if rws.UrlRelabelConfig != nil || len(rws.InlineUrlRelabelConfig) > 0 {
			...
			value = path.Join(victoriametricsv1beta1.RelabelingConfigDir, fmt.Sprintf(urlRelabelingName, i))
		}
		...
		if rws.HasStreamAggr() {
			...
			streamConfVal = path.Join(victoriametricsv1beta1.StreamAggrConfigDir, rws.AsConfigMapKey(i, "stream-aggr-conf"))
...
func buildConfigReloaderArgs(cr *victoriametricsv1beta1.VMAgent, c *config.BaseOperatorConf) []string {
	...
	if cr.HasAnyStreamAggrConfigs() {
		args = append(args, fmt.Sprintf("--%s=%s", dirsArg, victoriametricsv1beta1.StreamAggrConfigDir))
	}
	if cr.HasAnyRelabellingConfigs() {
		args = append(args, fmt.Sprintf("--%s=%s", dirsArg, victoriametricsv1beta1.RelabelingConfigDir))
	}
	if c.UseCustomConfigReloader {
