...
func (r *VMAnomalyModelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	for i := range objects.Items {
		...
		if !item.DeletionTimestamp.IsZero() || item.Spec.ParsingError != "" || item.IsModelUnmanaged() {
			continue
		}
		...
		if !instance.DeletionTimestamp.IsZero() {
			opts := &k8stools.SelectorOpts{
				NamespaceSelector: item.Spec.ModelNamespaceSelector,
				ObjectSelector:    item.Spec.ModelSelector,
