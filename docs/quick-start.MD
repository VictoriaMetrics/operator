...
If there is no secret, default secret will be created with predefined content. It is possbile to provide raw yaml config
to the alertmanager.
...
## VMNodeScrape
 ...
 `VMNodeScrape` is usefull for node exporters monitoring, lets create scraper for cadvisor metrics:
