	...
	f := func(cr *vmv1.VLCluster, validate func(ctx context.Context, rclient client.Client) error, predefinedObjects []runtime.Object) {
		...
		if cr.Spec.VLStorage != nil {
			...
			eventuallyUpdateStatusToOk(func() error {
				if err := fclient.Get(ctx, types.NamespacedName{Name: cr.PrefixedName(vmv1beta1.ClusterComponentStorage), Namespace: cr.Namespace}, &vlst); err != nil {
					return err
				}
		...
		if cr.Spec.VLSelect != nil {
			...
			eventuallyUpdateStatusToOk(func() error {
				if err := fclient.Get(ctx, types.NamespacedName{Name: cr.PrefixedName(vmv1beta1.ClusterComponentSelect), Namespace: cr.Namespace}, &vls); err != nil {
					return err
				}
		...
		if cr.Spec.VLInsert != nil {
			...
			eventuallyUpdateStatusToOk(func() error {
				if err := fclient.Get(ctx, types.NamespacedName{Name: cr.PrefixedName(vmv1beta1.ClusterComponentInsert), Namespace: cr.Namespace}, &vli); err != nil {
					return err
				}
		...
		if cr.Spec.RequestsLoadBalancer.Enabled {
			...
			eventuallyUpdateStatusToOk(func() error {
				if err := fclient.Get(ctx, types.NamespacedName{Name: cr.PrefixedName(vmv1beta1.ClusterComponentBalancer), Namespace: cr.Namespace}, &vmauthLB); err != nil {
					return err
				}
