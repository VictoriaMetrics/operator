...
func CreateVMSingleStorage(ctx context.Context, cr *victoriametricsv1beta1.VMSingle, rclient client.Client) error {
...
func makeSpecForVMSingle(ctx context.Context, cr *victoriametricsv1beta1.VMSingle, c *config.BaseOperatorConf) (*corev1.PodTemplateSpec, error) {
	...
	if cr.HasStreamAggrConfig() {
		...
		args = append(args, fmt.Sprintf("--streamAggr.config=%s", path.Join(victoriametricsv1beta1.StreamAggrConfigDir, "config.yaml")))
