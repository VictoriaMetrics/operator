...
`spec.targets.k8s[*].role` supports `ingress`, `service` and `pod` values.

For `pod` and `service` roles path and scheme are not exposed via metalabels and equal to `/` and `http` respectively by default.
