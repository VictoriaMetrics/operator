...
func convertSpec(chart Chart, defaults []byte, valuesFile, targetName, targetNamespace string) (any, error) {
	...
	if len(targetName) > maxTargetNameLength {
		return nil, fmt.Errorf("target name %q is %d characters, but the migrate tool's own derived names (e.g. the buffer agent's StatefulSet pod hostnames) must fit in Kubernetes's 63-character DNS label limit after prefixing — keep it at %d characters or fewer", targetName, len(targetName), maxTargetNameLength)
	}
