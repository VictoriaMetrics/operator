...
func TestWaitQueueDrained(t *testing.T) {
	f := func(t *testing.T, body string, wantErr bool) {
		...
		synctest.Test(t, func(t *testing.T) {
			...
			defer cancel()
			err := waitQueueDrained(ctx, c, handlerClient(handler), agent, vmv1beta1.VMAgentQueueMetricName)
			if wantErr {
				assert.Error(t, err)
			} else {
...
func TestWaitQueueDrained_PermanentStatusFailsFast(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		err := waitQueueDrained(ctx, c, handlerClient(handler), agent, vmv1beta1.VMAgentQueueMetricName)
		assert.Error(t, err)
		assert.Less(t, time.Since(start), DrainPollInterval*2)
	})
}
