...
func TestClusterEngine_SelectCacheVolume_NoCacheMountPathIsIgnored(t *testing.T) {
	target := &vmv1beta1.VMCluster{
		ObjectMeta: metav1.ObjectMeta{Name: "myrelease", Namespace: "default"},
		Spec: vmv1beta1.VMClusterSpec{
	...
	engine := clusterEngine()
	assert.Empty(t, engine.PVCTemplateName(target, vmv1beta1.ClusterComponentSelect), "select's cache PVC would never be mounted without cacheMountPath — migrating it would orphan the data")
}
...
func TestNoDowntimeCluster_PropagatesInsertPathPrefixToBufferAgent(t *testing.T) {
	synctest.Test(t, testNoDowntimeCluster_PropagatesInsertPathPrefixToBufferAgent)
}

func pathPrefixAwareAdminHandler(queueMetricName string) http.Handler {
	var scrapes int
...
func testNoDowntimeCluster_PropagatesInsertPathPrefixToBufferAgent(t *testing.T) {
	...
	testHTTPClient := migratetest.HandlerClient(pathPrefixAwareAdminHandler(vmv1beta1.VMAgentQueueMetricName))
	...
	oldInsertDep := &appsv1.Deployment{
		...
		Spec: appsv1.DeploymentSpec{
			...
			Template: corev1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"app": "old-vminsert"}},
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{{Name: "vminsert", Args: []string{"-http.pathPrefix=/foo"}}},
				},
			},
		},
	}
	...
	var agentExtraArgs map[string]string
	c := fake.NewClientBuilder().WithScheme(migratetest.Scheme(vmv1beta1.AddToScheme)).
		WithStatusSubresource(&vmv1beta1.VMCluster{}, &vmv1beta1.VMAgent{}, &corev1.PersistentVolumeClaim{}).
		WithObjects(oldInsertDep, oldInsertSvc, oldSelectSTS, oldSelectSvc, oldStorageSTS, oldStorageSvc, pv, pvc).
		WithInterceptorFuncs(interceptor.Funcs{
			Create: func(ctx context.Context, cl client.WithWatch, obj client.Object, opts ...client.CreateOption) error {
				if agent, ok := obj.(*vmv1beta1.VMAgent); ok && agentExtraArgs == nil {
					agentExtraArgs = agent.Spec.ExtraArgs
				}
				return cl.Create(ctx, obj, opts...)
			},
		}).
	...
	assert.Equal(t, map[string]string{"http.pathPrefix": "/foo"}, agentExtraArgs)
}
